...
/*************************************** gpio ***************************************/
// #define AD100_SLEEP_GPIO GPIO_PE(01)
...
static noinline void cpu_resume(void)
{
	...
	TCSM_DELAY(100);
#endif
#ifdef AD100_SLEEP_GPIO
	/* resume normal voltage */
	gpio_writel(1 << (AD100_SLEEP_GPIO % 32), GPIO_PXPAT0S(AD100_SLEEP_GPIO / 32));
...
static noinline void cpu_sleep(void)
{
	...
	if ((sleep_param->state == PM_SUSPEND_MEM) || (sleep_param->state == PM_SUSPEND_STANDBY)) {
		...
		*(volatile unsigned int *)0xb3012000 |= (1 << 3);
		...
		gpio_writel(1 << (AD100_SLEEP_GPIO % 32), GPIO_PXPAT0C(AD100_SLEEP_GPIO / 32));
#endif
...
#endif
int ad100_pm_enter(suspend_state_t state)
{
	//unsigned int cpu_id;

	printk("ad100 pm enter!!\n");
#if 1
...
static int ad100_pm_begin(suspend_state_t state)
{
	printk("ad100 suspend begin\n");
	return 0;
}

static void ad100_pm_end(void)
{
	printk("ad100 pm end!\n");
}
...
static const struct platform_suspend_ops ad100_pm_ops = {
	.valid      = ingenic_pm_valid,
	.begin      = ad100_pm_begin,
	.enter      = ad100_pm_enter,
	.end        = ad100_pm_end,
};
...
#ifdef AD100_SLEEP_GPIO
	gpio_request(AD100_SLEEP_GPIO, "sleep_gpio");
	gpio_direction_output(AD100_SLEEP_GPIO, 1);
#endif

	suspend_set_ops(&ad100_pm_ops);
