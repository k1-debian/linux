...
int avpu_codec_probe(struct platform_device *pdev)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,10, 0)
	/* willd be marked as uncahable  */
	codec->regs = devm_ioremap_wc(&pdev->dev,
	...
	ret = of_reserved_mem_device_init(codec->device);
	if (ret) {
		dev_warn(codec->device, "failed to init reserved mem\n");
	}
	return 0;
...
{
	module_class = class_create("avpu_class");
