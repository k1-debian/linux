...
#define PANEL_FW050_ID  (0x000000)
...
static struct dsi_cmd_packet fitipower_fw050_720_1280_2lane_cmd_list[] = {
...
static struct dsi_cmd_packet fitipower_fw050_720_1280_4lane_cmd_list[] = {
	{0x39, 0x04, 0x00, {0xFF, 0x98, 0x81, 0x03}},
	{0x39, 0x02, 0x00, {0x01, 0x00}},
	{0x39, 0x02, 0x00, {0x02, 0x00}},
...
static void panel_dev_panel_init(struct panel_dev *lcd)
{
	...
	if (dsi->video_config->no_of_lanes == 2) {
		param_list = &fitipower_fw050_720_1280_2lane_cmd_list[0];
		num_of_param = ARRAY_SIZE(fitipower_fw050_720_1280_2lane_cmd_list);
	} else {
		param_list = &fitipower_fw050_720_1280_4lane_cmd_list[0];
		num_of_param = ARRAY_SIZE(fitipower_fw050_720_1280_4lane_cmd_list);
	}
...
/*
    set_display_on 接口，用于控制屏幕显示和关闭，仅作调试使用.
*/
static int panel_set_display_on(struct mipi_dsim_lcd_device *dsim_dev, int on)
{
	if (on) {
		panel_dev_display_on(panel);
	} else {
...
static struct mipi_dsim_lcd_driver panel_dev_dsim_ddi_driver = {
	...
	.set_display_on = panel_set_display_on,
};
...
static int panel_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "Failed to parse panel!\n");
		goto err;
	}
