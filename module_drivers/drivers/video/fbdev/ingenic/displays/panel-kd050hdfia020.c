/*
 * driver/video/fbdev/ingenic/fb_stage/displays/panel-kd050hdfia020.c
...
static char panel_kd050hdfia020_debug = 0;
#define PANEL_DEBUG_MSG(msg...)         \
	do {                    \
		if (panel_kd050hdfia020_debug)      \
			printk(">>>>>>>>>>>>>>>>PANEL KD050HDFIA020: " msg);        \
	} while(0)
...
#define PANEL_KD050HDFIA020_ID  (0x98815c)

struct kd050hdfia020 {
	struct device *dev;
	unsigned int power;
	unsigned int id;
...
static struct dsi_cmd_packet fitipower_kd050hdfia020_720_1280_cmd_list[] = {
...
static int panel_dev_read_id(struct mipi_dsim_lcd_device *dsim_dev)
{
	...
	if (panel_id == PANEL_KD050HDFIA020_ID) {
		return 0;
	} else {
		return -1;
	}
...
static void panel_dev_panel_init(struct panel_dev *lcd)
{
	...
	for (i = 0; i < ARRAY_SIZE(fitipower_kd050hdfia020_720_1280_cmd_list); i++) {
		ret = ops->cmd_write(dsi,  fitipower_kd050hdfia020_720_1280_cmd_list[i]);
	}
}

static void panel_dev_set_sequence(struct mipi_dsim_lcd_device *dsim_dev)
{
	struct kd050hdfia020 *lcd = dev_get_drvdata(&dsim_dev->dev);
	PANEL_DEBUG_MSG("enter %s %d \n", __func__, __LINE__);
...
static struct fb_videomode panel_modes = {
	.name = "fitipower_kd050hdfia020-lcd",
	.xres = 720,
	.yres = 1280,
...
static struct tft_config kd050hdfia020_cfg = {
	.pix_clk_inv = 0,
	.de_dl = 0,
	.sync_dl = 0,
...
/*
* @ pannel_kd050hdfia020_lcd_ops, register to kernel common backlight/lcd.c framworks.
*/
...
{
	struct kd050hdfia020 *lcd;
	char buffer[4];
	int ret;

	lcd = devm_kzalloc(&dsim_dev->dev, sizeof(struct kd050hdfia020), GFP_KERNEL);
	if (!lcd) {
		dev_err(&dsim_dev->dev, "failed to allocate fitipower_kd050hdfia020 structure.\n");
		return -ENOMEM;
	}
	...
	lcd->ld = lcd_device_register("fitipower_kd050hdfia020", lcd->dev, lcd,
	                              &panel_lcd_ops);
	if (IS_ERR(lcd->ld)) {
	...
	dev_dbg(lcd->dev, "probed fitipower_kd050hdfia020 panel driver.\n");
...
static const struct of_device_id panel_of_match[] = {
	{ .compatible = "ingenic,kd050hdfia020", },
	{},
};
