...
#define MDEVNAME    "soc-nna"
#define CPU_SIMULATOR
#define CMD_LINE "/proc/cmdline"
#define SOC_NNA_VERSION "H20220525"
...
long soc_nna_flushcache(struct soc_nna *pnna, long usr_arg)
{
	...
	/* arch_sync_dma_for_device(paddr,info.len,info.dir); */
	if (info.dir == DMA_BIDIRECTIONAL) {
		dma_cache_wback_inv(info.addr, info.len);
	} else if (info.dir == DMA_TO_DEVICE) {
		dma_cache_wback(info.addr, info.len);
	} else {
		dma_cache_inv(info.addr, info.len);
	}
#else
...
{
	long ret = 0;
	uint32_t soc_version_buf = 0x20220525;
...
static int get_nmem_info(uint32_t *nmem_addr, uint32_t *nmem_size)
{
	...
	cmdline = kzalloc(512, GFP_KERNEL);
	if (!cmdline) {
		printk("%s: mpsys_kzalloc mem failed!\n", __func__);
		ret = -ENOMEM;
	} else {

		sprintf(cmdline, "%s", saved_command_line);
		//printk("%s\n", cmdline);
		p = strstr(cmdline, "nmem");
		if (p == NULL) {
...
static int soc_nna_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	unsigned int oram_size_cmd_t = 0, oram_real_size = 0;
	...
	/* l2cache_size + ORAM = 1*1024*1024 这里是获取l2cache size，根据l2cache size来得到ORAM size */
	oram_size_cmd_t = *((volatile unsigned int *)(0xb2200060));
	switch ((oram_size_cmd_t & 0x1c00) >> 10) {
		case 1:
			l2cache_size = 128;
			oram_real_size = 1024 - l2cache_size;
			break;
		case 2:
			l2cache_size = 256;
			oram_real_size = 1024 - l2cache_size;
	...
	if (extend == 0) {
		...
		unsigned int real_size = oram_real_size * 1024;//896 * 1024
		...
		//if(before_s > nmem_size)
		//  goto error_set_nmem_size;
...
static int soc_nna_probe(struct platform_device *pdev)
{
	...
	pdev->dev.dma_mask = &soc_nna_dma_mask;
	pdev->dev.coherent_dma_mask = 0xffffffff;

	pnna->mdev.minor = MISC_DYNAMIC_MINOR;
	pnna->mdev.name = MDEVNAME;
...
#endif
static struct of_device_id soc_nna_match[] = {
	{.compatible = "ingenic,soc-nna", .data = NULL},
	{ },
};
static struct platform_driver soc_nna_driver = {
	...
	.driver = {
		.name = "soc-nna",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(soc_nna_match),
#ifdef CONFIG_PM
		.pm = &soc_nna_pm_ops,
#endif
	},
