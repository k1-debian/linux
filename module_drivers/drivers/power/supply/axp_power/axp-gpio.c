/*
 * axp809x-gpio.c  --  gpiolib support for Krosspower &axp PMICs
 *
...
#define GPIO_AXP809(n)      (AXP_NR_BASE + (n))
...
int axp_gpio_set_io(int gpio, int io_mode)
{
	if (io_mode == 1) { //output
		switch (gpio) {
		case 0: return axp_clr_bits(&axp->dev, AXP_GPIO0_CFG, 0x06);
		case 1: return axp_clr_bits(&axp->dev, AXP_GPIO1_CFG, 0x06);
		case 2: return 0;
		...
		switch (gpio) {
		case 0: axp_clr_bits(&axp->dev, AXP_GPIO0_CFG, 0x05);
			return axp_set_bits(&axp->dev, AXP_GPIO0_CFG, 0x02);
		case 1: axp_clr_bits(&axp->dev, AXP_GPIO1_CFG, 0x05);
			return axp_set_bits(&axp->dev, AXP_GPIO1_CFG, 0x02);
		case 2:
...
int axp_gpio_get_io(int gpio, int *io_mode)
{
	uint8_t val;
	switch (gpio) {
	case 0: axp_read(&axp->dev, AXP_GPIO0_CFG, &val); val &= 0x07;
		if (val < 0x02) {
			*io_mode = 1;
		} else if (val == 0x02) {
			*io_mode = 0;
		...
		break;
	case 1: axp_read(&axp->dev, AXP_GPIO1_CFG, &val); val &= 0x07;
		if (val < 0x02) {
			*io_mode = 1;
		} else if (val == 0x02) {
			*io_mode = 0;
	...
	case 2: *io_mode = 1;
		break;
	case 3: axp_read(&axp->dev, AXP_GPIO3_CFG, &val); val &= 0x08;
		if (!val) {
			*io_mode = 1;
		} else {
			return -EIO;
		...
		break;
	case 4: axp_read(&axp->dev, AXP_GPIO4_CFG, &val); val &= 0x10;
		if (!val) {
			*io_mode = 1;
		} else {
			return -EIO;
...
int axp_gpio_set_value(int gpio, int value)
{
	...
	if (1 == io_mode) { //output
		if (value) { //high
			switch (gpio) {
			case 0: {
				axp_clr_bits(&axp->dev, AXP_GPIO0_CFG, 0x06);
				return axp_set_bits(&axp->dev, AXP_GPIO0_CFG, 0x01);
			}
			case 1: {
				axp_clr_bits(&axp->dev, AXP_GPIO1_CFG, 0x06);
				ret = axp_set_bits(&axp->dev, AXP_GPIO1_CFG, 0x01);
				return ret;
			}
			case 2: {
			...
			case 3: {
				axp_clr_bits(&axp->dev, AXP_GPIO3_CFG, 0x08);
				ret = axp_clr_bits(&axp->dev, AXP_GPIO3_CFG, 0x30);
				return ret;
			}
			case 4: {
			...
			switch (gpio) {
			case 0: axp_clr_bits(&axp->dev, AXP_GPIO0_CFG, 0x06);
				return axp_clr_bits(&axp->dev, AXP_GPIO0_CFG, 0x03);
			case 1: axp_clr_bits(&axp->dev, AXP_GPIO1_CFG, 0x06);
				return axp_clr_bits(&axp->dev, AXP_GPIO1_CFG, 0x03);
			case 2: return axp_clr_bits(&axp->dev, AXP_GPIO2_CFG, 0x80);
			case 3: axp_clr_bits(&axp->dev, AXP_GPIO3_CFG, 0x08);
				return axp_set_bits(&axp->dev, AXP_GPIO3_CFG, 0x30);
			case 4: axp_clr_bits(&axp->dev, AXP_GPIO4_CFG, 0x10);
...
int axp_gpio_get_value(int gpio, int *value)
{
	...
	if (1 == io_mode) { //output
		switch (gpio) {
		case 0:
			ret = axp_read(&axp->dev, AXP_GPIO0_CFG, &val);
			if (ret) {
				printk("Read AXP_GPIO0 value fail! ");
				*value = 0;
				return -ENXIO;
			} else {
				*value = val & 0x01;
			...
			break;
		case 1:
			ret = axp_read(&axp->dev, AXP_GPIO1_CFG, &val);
			if (ret) {
				printk("Read AXP_GPIO1 value fail! ");
				*value = 0;
				return -ENXIO;
			}
			*value = val & 0x01;
			break;
		case 2:
			ret = axp_read(&axp->dev, AXP_GPIO2_CFG, &val);
			if (ret) {
				printk("Read AXP_GPIO2 value fail! ");
				*value = 0;
				return -ENXIO;
			}
			*value = ((val & 0x80) >> 7);
			break;
		case 3:
			ret = axp_read(&axp->dev, AXP_GPIO3_CFG, &val);
			if (ret) {
				printk("Read AXP_GPIO3 value fail! ");
				*value = 0;
				return -ENXIO;
			}
			if ((val & 0x30) == 0x30) {
			...
			break;
		case 4:
			ret = axp_read(&axp->dev, AXP_GPIO4_STA, &val);
			if (ret) {
				printk("Read AXP_GPIO4 value fail! ");
				*value = 0;
				return -ENXIO;
			}
			*value = ((val & 0x04) >> 2);
		...
		switch (gpio) {
		case 0: ret = axp_read(&axp->dev, AXP_GPIO01_STATE, &val); val &= 0x10; *value = val >> 4; break;
		case 1: ret = axp_read(&axp->dev, AXP_GPIO01_STATE, &val); val &= 0x20; *value = val >> 5; break;
...
static int __axp_gpio_input(struct gpio_chip *chip, unsigned offset)
{
	...
	if (GPIO_AXP809(0) == index) {
		return axp_gpio_set_io(0, 0);
	} else if (GPIO_AXP809(1) == index) {
		return axp_gpio_set_io(1, 0);
	} else if (GPIO_AXP809(2) == index || GPIO_AXP809(3) == index || GPIO_AXP809(4) == index) {
...
static int __axp_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	...
	if (GPIO_AXP809(0) == index) {
		...
		return value;
	} else if (GPIO_AXP809(2) == index) {
		WARN_ON(0 != axp_gpio_get_value(2, &value));
		return value;
	} else if (GPIO_AXP809(3) == index) {
		WARN_ON(0 != axp_gpio_get_value(3, &value));
		return value;
	} else if (GPIO_AXP809(4) == index) {
