...
#if defined(CONFIG_INGENIC_PDMAC)
extern u32 ingenic_dma_read_dda(struct dma_chan *chan);
extern int ingenic_dma_get_desc_index(struct dma_chan *chan, unsigned int doa, int desc_num);
extern u32 ingenic_dma_read_dtc(struct dma_chan *chan);
#else
...
struct uart_ingenic_port {
	...
	struct uart_pdma_info pdma_info;
...
static void ingenic_get_doa_and_dtc(struct uart_ingenic_port *up)
{
	unsigned int doa, dda;

	dda = ingenic_dma_read_dda(up->chan_rx);
	doa = (dda >> 4) & 0xff;

	do {
		up->pdma_info.doa = doa;
		up->pdma_info.dtc = ingenic_dma_read_dtc(up->chan_rx);

		dda = ingenic_dma_read_dda(up->chan_rx);
		doa = (dda >> 4) & 0xff;
	} while (up->pdma_info.doa != doa);
}

static void serial_ingenic_insert_data(struct uart_ingenic_port *up)
{
	int i, start, end;
	...
	up->pdma_info.recv_len = 0;

	if (up->pdma_info.idx != up->pdma_info.idx_last || up->pdma_info.tc != up->pdma_info.tc_last) {
		start = up->pdma_info.idx_last * DESC_SIZE + DESC_SIZE - up->pdma_info.tc_last;
		end = up->pdma_info.idx * DESC_SIZE + DESC_SIZE - up->pdma_info.tc;

		if (end > start) {
			up->pdma_info.recv_len = end - start;
		} else {
			up->pdma_info.recv_len = DESC_SIZE * DESC_NUM - start + end;
		}
	}
	/*
	   printk("%s: recv_len = %d, s: %d(%d, %d), e: %d(%d, %d)\n", __func__,
	           up->pdma_info.recv_len, start, up->pdma_info.idx_last, up->pdma_info.tc_last,
	           end, up->pdma_info.idx, up->pdma_info.tc);
	*/
	if (up->pdma_info.recv_len) {
		if (up->pdma_info.recv_len <= 0) {
			up->pdma_info.err++;
		}

		for (i = 0; i < up->pdma_info.recv_len; i++) {
			offset = (start + i) % (DESC_SIZE * DESC_NUM);
			uart_insert_char(&up->port, status, UART_LSR_OE, ((unsigned char *)up->rx_buf_virt)[offset], TTY_NORMAL);
		}
		port->icount.rx += up->pdma_info.recv_len;
		tty_flip_buffer_push(tty->port);

		up->pdma_info.tc_last = up->pdma_info.tc;
		up->pdma_info.idx_last = up->pdma_info.idx;
	}
}

static void serial_ingenic_get_index_and_dtc(struct uart_ingenic_port *up)
{
	int ret;

	do {
		/* Get index of dma desc */
		ingenic_get_doa_and_dtc(up);
		ret = ingenic_dma_get_desc_index(up->chan_rx, up->pdma_info.doa, DESC_NUM);
		if (ret < 0) {
			if (up->pdma_info.doa == 0) {
				up->pdma_info.idx = 0;
			} else {
				up->pdma_info.err++;
			}
		} else {
			up->pdma_info.idx = ret;
		}

		/* Get dtc of dma desc */
		up->pdma_info.tc = up->pdma_info.dtc;
	} while ((serial_in(up, UART_IIR) & UART_ISR_MASK) == UART_ISR_IID_RETO);
}

static void serial_ingenic_recv_data(struct uart_ingenic_port *up)
{
	unsigned long flags;

	spin_lock_irqsave(&up->port.lock, flags);
	serial_ingenic_get_index_and_dtc(up);
	serial_ingenic_insert_data(up);
	spin_unlock_irqrestore(&up->port.lock, flags);
}

static void serial_ingenic_error_handler(struct uart_ingenic_port *up, unsigned int lsr)
{
	...
	while ((lsr & UART_LSR_FIFOE) && (lsr & UART_LSR_DR)) {
		...
		if (unlikely(status & (UART_LSR_BI | UART_LSR_PE | UART_LSR_FE | UART_LSR_OE))) {
			if (status & UART_LSR_BI) {
				dev_err(up->port.dev, "%s():%d >>> break error: %d\n",
				...
				*/
				...
				dev_err(up->port.dev, "%s():%d >>> parity error: %d\n",
				        __func__, __LINE__, ch);
				up->port.icount.parity++;
			} else if (status & UART_LSR_FE) {
				dev_err(up->port.dev, "%s():%d >>> frame error: %d\n",
				        __func__, __LINE__, ch);
...
ignore_char:
	...
	up->pdma_info.idx = 0;
	up->pdma_info.idx_last = 0;
	up->pdma_info.tc = DESC_SIZE;
	up->pdma_info.tc_last = DESC_SIZE;

	dma_async_issue_pending(up->chan_rx);
}

static void ingenic_dma_rx_complete(void *arg)
{
	struct uart_ingenic_port *up = arg;
	struct uart_port *port = &up->port;
	struct tty_struct *tty = tty_port_tty_get(&port->state->port);

	if (!tty) {
		dev_err(up->port.dev, "%s:tty is busy now", __func__);
		return;
	}

	/* printk("%s\n", __func__); */
	serial_ingenic_recv_data(up);

	tty_kref_put(tty); //Free TTY quote
	async_tx_ack(up->desc_rx);
}

static void ingenic_dma_rx(unsigned long data)
{
	struct uart_ingenic_port *up = (struct uart_ingenic_port *)data;

	if (!up->chan_rx) {
		printk("no dma rx channel\n");
		return;
	}

	memset(up->rx_buf_virt, 0, (DESC_NUM * DESC_SIZE));

	up->desc_rx = dmaengine_prep_dma_cyclic(up->chan_rx,
	                                        up->rx_buf_dma,
	                                        DESC_NUM * DESC_SIZE,
	                                        DESC_SIZE,
	                                        DMA_DEV_TO_MEM,
	...
	up->desc_rx->callback = ingenic_dma_rx_complete;
	up->desc_rx->callback_param = up;

	dmaengine_submit(up->desc_rx);
	dma_async_issue_pending(up->chan_rx);
}
...
*/
static void ingenic_dma_tx_complete(void *arg)
{
	struct uart_ingenic_port *up = arg;
	struct circ_buf *xmit = &up->port.state->xmit;

	dma_unmap_sg(up->port.dev, &up->sg_tx, 1, (enum dma_data_direction)DMA_MEM_TO_DEV);
	if (!(uart_tx_stopped(&up->port) || uart_circ_empty(xmit))) {
		tasklet_schedule(&up->tasklet_dma_tx);
	}
}

static void ingenic_dma_tx(unsigned long data)
{
	struct uart_ingenic_port *up = (struct uart_ingenic_port *)data;
	struct circ_buf *xmit = &up->port.state->xmit;
	int bytes, lsr;
	...
	dma_map_sg(up->port.dev, &up->sg_tx, 1, (enum dma_data_direction)DMA_MEM_TO_DEV);
	up->desc_tx = dmaengine_prep_slave_sg(up->chan_tx,
	                                      &up->sg_tx,
	                                      1,
	                                      DMA_MEM_TO_DEV,
	                                      DMA_PREP_INTERRUPT | DMA_CTRL_ACK);
	...
	up->desc_tx->callback = ingenic_dma_tx_complete;
...
static int ingenic_uart_dma_rxtx_config(struct uart_ingenic_port *up)
{

	up->rx_dma_config.src_addr = (dma_addr_t)(up->port.mapbase + UART_RX);
	up->rx_dma_config.src_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
	up->rx_dma_config.dst_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
	up->rx_dma_config.src_maxburst = 16;
	up->rx_dma_config.dst_maxburst = 16;
	//up->rx_dma_config.slave_id = 0;
	up->rx_dma_config.direction = DMA_DEV_TO_MEM;
	dmaengine_slave_config(up->chan_rx, &up->rx_dma_config);

	up->tx_dma_config.dst_addr = (dma_addr_t)(up->port.mapbase + UART_TX);
	up->tx_dma_config.dst_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
	up->tx_dma_config.src_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
	up->tx_dma_config.dst_maxburst = 32;
	up->tx_dma_config.src_maxburst = 32;
	//up->tx_dma_config.slave_id = 0;
	up->tx_dma_config.direction = DMA_MEM_TO_DEV;
	dmaengine_slave_config(up->chan_tx, &up->tx_dma_config);
...
static inline irqreturn_t serial_ingenic_irq(int irq, void *dev_id)
{
	...
	if (up->use_dma) {
		serial_out(up, UART_IER, 0);     /* disable_irq */

		if ((iir & UART_ISR_MASK) == UART_ISR_IID_RETO) {
			/* printk("%s\n", __func__); */
			serial_ingenic_recv_data(up);
		} else if ((iir & UART_ISR_MASK) == UART_ISR_IID_ERR) {
			serial_ingenic_error_handler(up, lsr);
		}
...
static int serial_ingenic_startup(struct uart_port *port)
{
	...
	if (up->use_dma) {
		ingenic_uart_dma_rxtx_config(up);

		if (!up->pdma_info.once) {
			up->pdma_info.once = 1;
	 ...
	 */
	if (up->use_dma) {
		// serial_out(up, UART_URFCR, 0x7f);
		up->ier = UART_IER_RLSI | UART_IER_RETOIE;
	} else {
...
static void serial_ingenic_set_termios(struct uart_port *port, struct ktermios *termios, const struct ktermios *old)
{
	struct uart_ingenic_port *up = (struct uart_ingenic_port *)port;
...
static void serial_ingenic_console_putchar(struct uart_port *port, unsigned char ch)
{
	struct uart_ingenic_port *up = (struct uart_ingenic_port *)port;
...
static int serial_ingenic_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(np, "dma-mode")) {
		up->use_dma = 1;
	} else {
		up->use_dma = 0;
	}
	...
	up->port.has_sysrq = IS_ENABLED(CONFIG_SERIAL_INGENIC_CONSOLE);
	...
	if (up->use_dma) {
		...
		dev_info(up->port.dev, "Current %s is DMA mode...\n", up->name);

		dma_cap_zero(mask);
		dma_cap_set(DMA_SLAVE, mask);

		up->chan_rx = dma_request_chan(&pdev->dev, "rx");
		if (up->chan_rx < 0) {
		...
		up->chan_tx = dma_request_chan(&pdev->dev, "tx");
		if (up->chan_tx < 0) {
		...
		up->rx_buf_virt = dma_alloc_coherent(up->port.dev, (DESC_NUM * DESC_SIZE),
		                                     &up->rx_buf_dma, GFP_KERNEL);
		if (!up->rx_buf_virt) {
			dev_err(up->port.dev, "up->rx_buf_virt fail\n");
			return 0;
		}

		tasklet_init(&up->tasklet_dma_tx, ingenic_dma_tx, (unsigned long)up);
		tasklet_init(&up->tasklet_dma_rx, ingenic_dma_rx, (unsigned long)up);
