...
static int ov7251_probe(struct i2c_client *client)
{
	...
	if (gpio_is_valid(gpio)) {
		...
		info->ircutn.active_level = INGENIC_ISP_GPIO_ACTIVE_HIGH;
	}

	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,pwr-gpio", 0);
	if (gpio_is_valid(gpio)) {
		info->pwr.pin = gpio;
		info->pwr.active_level = INGENIC_ISP_GPIO_ACTIVE_HIGH;
	}
