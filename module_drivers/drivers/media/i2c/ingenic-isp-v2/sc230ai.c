...
static struct regval_list sc230ai_init_regs_1920_1080_30fps_wdr_MIPI[] = {
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
	...
	{0x3e01, 0x06},
	{0x3e02, 0x00},
	//  {0x3e22,0x00}, //short exp
	{0x3e04, 0x10}, //short exp
	{0x3e05, 0x60}, //short exp
	{0x3e06, 0x00},
	{0x3e07, 0x80},
	{0x3e08, 0x03},
	{0x3e09, 0x40},
...
static struct sc230ai_win_size sc230ai_win_sizes[] = {
	...
	},
	{
		...
		.sensor_info.fps                = 30 << 16 | 1,
		                                     ...
		                                     .ycbcr_enc      = V4L2_YCBCR_ENC_DEFAULT,
		                                     .quantization   = V4L2_QUANTIZATION_FULL_RANGE,
		                                     .regs       = sc230ai_init_regs_1920_1080_30fps_wdr_MIPI,
	},
};
...
{
	struct sc230ai_info *info = to_state(sd);
	struct sc230ai_win_size *wsize = info->win;
...
static int sc230ai_s_wdr(struct v4l2_subdev *sd, int value)
{
	int ret = 0;
	struct sc230ai_info *info = to_state(sd);
	//  printk("%s:%d\n", __func__, value);
	if (value) {
		info->win = &sc230ai_win_sizes[2];
	} else {
...
static int sc230ai_g_again_short(struct v4l2_subdev *sd, __s32 *value)
{
	char v = 0;
	unsigned int reg_val = 0;
	unsigned int fine_reg_val = 0;
	...
	ret = sc230ai_read(sd, 0x3e13, &v);
	reg_val = v ;
	ret = sc230ai_read(sd, 0x3e11, &v);
	fine_reg_val = v ;
...
static int sc230ai_s_again_short(struct v4l2_subdev *sd, int value)
{
	struct sc230ai_info *info = to_state(sd);
	unsigned int reg_value;
	unsigned int reg_fine_value;
	...
	ret += sc230ai_write(sd, 0x3e13, (unsigned char)(reg_value));
	ret += sc230ai_write(sd, 0x3e11, (unsigned char)(reg_fine_value));
	if (ret < 0) {
		printk("sc230ai_write error  %d\n", __LINE__);
		return ret;
	}
...
static int sc230ai_s_exp_short(struct v4l2_subdev *sd, int value)
{
	int ret = 0;

	value *= 2; /*unit in half line*/
	ret += sc230ai_write(sd, 0x3e22, (unsigned char)((value >> 12) & 0xf));
	ret += sc230ai_write(sd, 0x3e04, (unsigned char)((value >> 4) & 0xff));
	ret += sc230ai_write(sd, 0x3e05, (unsigned char)(value & 0xf) << 4);
...
static int sc230ai_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	switch (ctrl->id) {
		...
		case V4L2_CID_USER_ANALOG_GAIN_SHORT:
			return sc230ai_g_again_short(sd, &info->again_short->val);
	}
	return -EINVAL;
}
...
static int sc230ai_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	switch (ctrl->id) {
		...
		case V4L2_CID_USER_ANALOG_GAIN_SHORT:
			return sc230ai_s_again_short(sd, ctrl->val);
		case V4L2_CID_EXPOSURE:
			return sc230ai_s_exp(sd, ctrl->val);
		case V4L2_CID_USER_EXPOSURE_SHORT:
			return sc230ai_s_exp_short(sd, ctrl->val);
		case V4L2_CID_WIDE_DYNAMIC_RANGE:
			return sc230ai_s_wdr(sd, ctrl->val);
	}
	return -EINVAL;
}
...
static int sc230ai_core_reset(struct v4l2_subdev *sd, u32 val)
{
	unsigned char v;
	int ret;
...
static int sc230ai_core_init(struct v4l2_subdev *sd, u32 val)
{
	struct sc230ai_info *info = to_state(sd);
	int ret = 0;
...
#endif
	.init = sc230ai_core_init,
	.reset = sc230ai_core_reset,
...
static int sc230ai_probe(struct i2c_client *client)
{
	...
	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,xshutdn-gpio", 0);
	if (gpio_is_valid(gpio)) {
		info->xshutdn.pin = gpio;
		info->xshutdn.active_level = INGENIC_ISP_GPIO_ACTIVE_HIGH;
	}
	...
	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,efsync-gpio", 0);
	if (gpio_is_valid(gpio)) {
		info->efsync.pin = gpio;
		info->efsync.active_level = INGENIC_ISP_GPIO_ACTIVE_HIGH;
	}
	...
	info->exposure = v4l2_ctrl_new_std(&info->hdl, &sc230ai_ctrl_ops,
	                                   V4L2_CID_EXPOSURE, 1,  1121, 4, 600);

	cfg.ops = &sc230ai_ctrl_ops;
