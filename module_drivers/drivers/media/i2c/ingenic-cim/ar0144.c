...
static const struct ar0144_win_size *ar0144_select_win(u32 *code, u32 *width, u32 *height)
{
	...
	return &ar0144_supported_win_sizes[default_size];
}
                          ...
                          struct v4l2_subdev_format *format)
{
	...
	*(unsigned int *)mf->reserved = (unsigned int)&priv->win->sensor_info;
...
static void ar0144_remove(struct i2c_client *client)
{
	...
	devm_clk_put(&client->dev, priv->clk);
	devm_clk_put(&client->dev, priv->mclk);
	if(sd_pdata && sd_pdata->regulators)
		devm_kfree(&client->dev, sd_pdata->regulators);
	if(sd_pdata)
		devm_kfree(&client->dev, sd_pdata);
	devm_kfree(&client->dev, priv);
