...
struct ov4689_info {
	...
	struct gpio_desc *ircutp;
	struct gpio_desc *ircutn;
...
static struct regval_list ov4689_init_regs_1920_1080_120fps_mipi[] = {
...
#endif
#if 0
	/*support by ov , dosen't word in zebra & halley5*/
	//  @@ 0 35 RES_1920x1080_30fps_1008Mbps 4lane
	//100 99 1920 1080
	//102 3601 2ee0
	{ 0x0103, 0x01 },
	{ 0x3638, 0x00 },
...
static int ov4689_reset(struct v4l2_subdev *sd, u32 val)
{
	...
	if (val) {
		gpiod_direction_output(info->reset, 1);
		msleep(10);
		gpiod_direction_output(info->reset, 0);
		msleep(10);
	}
	return 0;
}
...
static int ov4689_ircut(struct v4l2_subdev *sd, u32 val)
{
	...
	if (val) {
		gpiod_direction_output(info->ircutp, 1);
		gpiod_direction_output(info->ircutn, 1);
		msleep(10);
		gpiod_direction_output(info->ircutp, 0);
	} else {
		gpiod_direction_output(info->ircutp, 0);
		gpiod_direction_output(info->ircutn, 0);
		msleep(10);
		gpiod_direction_output(info->ircutp, 1);
	}
	return 0;
...
static struct ov4689_win_size ov4689_win_sizes[] = {
	{
		...
		.regs       = ov4689_init_regs_1920_1080_120fps_mipi,
#else
...
#define N_OV4689_FMTS  ARRAY_SIZE(ov4689_win_sizes)
                                 ...
                                 struct v4l2_subdev_mbus_code_enum *code)
{
	...
	code->code = ov4689_win_sizes[code->index].mbus_code;
...
static int ov4689_g_mbus_config(struct v4l2_subdev *sd, unsigned int pad,
                        ...
                        const struct i2c_device_id *id)
{
	...
	info->reset = devm_gpiod_get_optional(&client->dev, "reset",
	                                      GPIOD_OUT_LOW);
	if (!info->reset) {
		dev_dbg(&client->dev, "reset gpio is not assigned!\n");
	} else if (IS_ERR(info->reset)) {
		return PTR_ERR(info->reset);
	}

	info->ircutp = devm_gpiod_get_optional(&client->dev, "ircutp",
	                                       GPIOD_OUT_LOW);
	if (!info->ircutp) {
		dev_dbg(&client->dev, "ircutp gpio is not assigned!\n");
	} else if (IS_ERR(info->ircutp)) {
		return PTR_ERR(info->ircutp);
	}

	info->ircutn = devm_gpiod_get_optional(&client->dev, "ircutn",
	                                       GPIOD_OUT_LOW);
	if (!info->ircutn) {
		dev_dbg(&client->dev, "ircutn gpio is not assigned!\n");
	} else if (IS_ERR(info->ircutn)) {
		return PTR_ERR(info->ircutn);
	}
...
#ifdef CONFIG_OV4689_2048X1520_30FPS
