...
static int sc0132gs_write_I2cRegs(struct v4l2_subdev *sd, TOMATO_I2C_regs *vals)
{
	int ret;
	int i;
	struct regval_list *pregs = vals->regs;
...
static int sc0132gs_probe(struct i2c_client *client)
{
	...
	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,vcc-en-gpio", 0);
	printk("[%d] gpio=%d\n", __LINE__, gpio);
	if (gpio_is_valid(gpio)) {
		info->vcc_en.pin = gpio;
		info->vcc_en.active_level = INGENIC_ISP_GPIO_ACTIVE_LOW;
	}
	...
	if (gpio_is_valid(gpio)) {
		...
		info->ircutp.active_level = INGENIC_ISP_GPIO_ACTIVE_LOW;
	}
	...
	if (i2c_adap) {
		led_ic_client = i2c_new_scanned_device(i2c_adap, &aw9110b_info, addr_list, NULL);
...
static void sc0132gs_remove(struct i2c_client *client)
{
