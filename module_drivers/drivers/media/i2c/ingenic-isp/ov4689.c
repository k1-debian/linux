...
static struct ov4689_win_size ov4689_win_sizes[] = {
	{
		...
		.sensor_info.mipi_cfg.clk       = 408,
		.regs       = ov4689_init_regs_1920_1080_30fps_mipi,
	},
	{
		.sensor_info.mipi_cfg.clk       = 300,
		.sensor_info.mipi_cfg.twidth        = 2048,
...
static int ov4689_power(struct v4l2_subdev *sd, int on)
{
	struct ov4689_info *info = to_state(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
	...
	if (IS_ERR(info->supply.avdd) || IS_ERR(info->supply.dvdd)
		...
		printk("No ov4689 vdd regulator found\n");
	}
	...
	if ((!IS_ERR(info->supply.avdd)) && (!IS_ERR(info->supply.avdd))
		...
		if (on) {
			...
			if (ret) {
				dev_err(&client->dev, "ov4689 vdd supply disable failed\n");
			}
		} else {
		...
		dev_err(&client->dev, "ov4689 vdd supply IS_ERR failed\n");
	}
	return ret;
}
...
const struct dev_pm_ops ov4689_pm = {
