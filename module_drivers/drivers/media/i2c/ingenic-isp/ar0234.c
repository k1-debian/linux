...
static int ar0234_probe(struct i2c_client *client)
{
	...
	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,expo-gpio", 0);
	if (gpio_is_valid(gpio)) {
		info->expo.pin = gpio;
		info->expo.active_level = INGENIC_ISP_GPIO_ACTIVE_LOW;
	}
	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,oe-gpio", 0);
	if (gpio_is_valid(gpio)) {
		info->oe.pin = gpio;
		info->oe.active_level = INGENIC_ISP_GPIO_ACTIVE_LOW;
	}
	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,pwen-gpio", 0);
	if (gpio_is_valid(gpio)) {
		info->pwen.pin = gpio;
		info->pwen.active_level = INGENIC_ISP_GPIO_ACTIVE_HIGH;
	}
	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,shutdown-gpio", 0);
	if (gpio_is_valid(gpio)) {
		info->shutdown.pin = gpio;
		info->shutdown.active_level = 0;
	}
