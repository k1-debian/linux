...
static int hm2140_probe(struct i2c_client *client)
{
	...
	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,sync-gpio", 0);
	if (gpio_is_valid(gpio)) {
		info->sync.pin = gpio;
		info->sync.active_level = INGENIC_ISP_GPIO_ACTIVE_LOW;
	}
...
static void hm2140_remove(struct i2c_client *client)
{
