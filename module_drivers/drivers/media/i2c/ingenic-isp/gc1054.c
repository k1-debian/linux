...
static struct gc1054_win_size *gc1054_select_win(u32 *width, u32 *height)
{
...
static int gc1054_probe(struct i2c_client *client)
{
	...
	//      info->reset.pin = gpio;
	//      info->reset.active_level = (flags == INGENIC_ISP_GPIO_ACTIVE_LOW) ? 0 : 1;
	...
	gpio = of_get_named_gpio(client->dev.of_node, "ingenic,cim-en-gpio", 0);
	if (gpio_is_valid(gpio)) {
		info->cimen.pin = gpio;
		info->cimen.active_level = INGENIC_ISP_GPIO_ACTIVE_LOW;
...
static void gc1054_remove(struct i2c_client *client)
{
