...
static int ov2735a_power_init(struct v4l2_subdev *sd)
{
	struct ov2735a_info *info = to_state(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);

	info->supply.avdd  = regulator_get(&client->dev, "avdd");
	info->supply.dvdd  = regulator_get(&client->dev, "dvdd");
	info->supply.dovdd = regulator_get(&client->dev, "dovdd");

	return 0;
}

static int ov2735a_power_deinit(struct v4l2_subdev *sd)
{
	struct ov2735a_info *info = to_state(sd);
	if (info->supply.avdd) {
		regulator_put(info->supply.avdd);
	}
	if (info->supply.dvdd) {
		regulator_put(info->supply.dvdd);
	}
	if (info->supply.dovdd) {
		regulator_put(info->supply.dovdd);
	}
...
static void ov2735a_remove(struct i2c_client *client)
{
