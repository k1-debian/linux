...
static unsigned int get_user_paddr(unsigned int vaddr)
{
	...
	// TODO: 如果是kmap的地址，比如C空间的地址这里需要处理一下使用pte_offset_kmap.
	pte = pte_offset_kernel(pmdir, vaddr);
	if (pte_present(*pte)) {
		return pte_pfn(*pte) << PAGE_SHIFT | offset_in_page(vaddr);
	}
	return 0;
}
                                ...
                                unsigned long vaddr, unsigned long size)
{
	...
	buf->vb = vb;
	buf->size = size;
	buf->dma_addr = get_user_paddr(vaddr);
...
EXPORT_SYMBOL_GPL(ingenic_vb2_dma_contig_set_max_seg_size);
