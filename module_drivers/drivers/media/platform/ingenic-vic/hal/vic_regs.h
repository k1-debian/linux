...
#define MIPI_VCROP_CHO_all_image_width  16, 31
#define MIPI_VCROP_CHO_start_pixel      0,  15
...
extern unsigned long vic_iobase_array[];

#define VIC_ADDR(index, reg)               ((volatile unsigned long *)(vic_iobase_array[index] + (reg)))
...
{
	*VIC_ADDR(index, reg) = set_bit_field(*VIC_ADDR(index, reg), start, end, val);
}
