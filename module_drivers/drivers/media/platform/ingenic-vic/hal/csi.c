...
static unsigned long mipi_csi_iobase[] = {
...
{
	*MIPI_CSI_ADDR(index, reg) = set_bit_field(*MIPI_CSI_ADDR(index, reg), start, end, val);
}
...
static unsigned long mipi_phy_iobase;

/*
 * MIPI PHY (Only One PHY)
 */
#define MIPI_PHY_ADDR(reg)              ((volatile unsigned long *)(mipi_phy_iobase + (reg)))
...
{
	*MIPI_PHY_ADDR(reg) = set_bit_field(*MIPI_PHY_ADDR(reg), start, end, val);
}
...
static void mipi_csi_gate_clock_enable(int index)
{
	struct jz_mipi_csi_drv *drv = &mipi_csi_dev[index];

	clk_prepare_enable(drv->csi_gate_clk);
}

static void mipi_csi_gate_clock_disable(int index)
{
	struct jz_mipi_csi_drv *drv = &mipi_csi_dev[index];

	clk_disable_unprepare(drv->csi_gate_clk);
}
...
static int mipi_csi_phy_configure(int index, struct mipi_bus *mipi_info)
{
	int ret = 0;
	unsigned long flags;
	spin_lock_irqsave(&csi_reset_lock, flags);

	int lanes = mipi_info->data_lanes;
...
int mipi_csi_phy_initialization(int index, struct mipi_bus *mipi_info)
{
	int ret = 0;
	int csi_lanes = mipi_info->data_lanes;
...
void mipi_csi_init_iodata(
    unsigned long iobase0, unsigned long iobase1, unsigned long phy_iobase,
    struct clk *gate_clk0, struct clk *gate_clk1)
{
	mipi_csi_iobase[0] = iobase0;
	mipi_csi_iobase[1] = iobase1;
	mipi_phy_iobase = phy_iobase;
	mipi_csi_dev[0].csi_gate_clk = gate_clk0;
	mipi_csi_dev[1].csi_gate_clk = gate_clk1;
}
