...
__attribute__((__unused__)) static int start_h264e_server(void)
{
#if 0
...
int h264e_nl_init_ctx(struct h264e_ctx *ctx, struct nl_ctx_info *info)
{
	...
	msg->ctx_id = (unsigned int)ctx;  /*以ctx的地址作为ctx_id*/
...
int h264e_nl_rc_video_cfg(struct h264e_ctx *ctx, struct h264e_params *p)
{
	...
	((struct h264e_params *)(msg->buf))->height = ALIGN(p->height, 8);
	((struct h264e_params *)(msg->buf))->width = ALIGN(p->width, 8);
