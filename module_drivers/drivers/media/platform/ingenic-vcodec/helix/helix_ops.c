...
#define H264_ENCODER_VERSION 0x00
#define JPEG_ENCODER_VERSION 0x00
#define JPEG_DECODER_VERSION 0x01
...
static const struct ingenic_codec_framesizes ingenic_venc_framesizes[] = {
	{
		.fourcc = V4L2_PIX_FMT_H264,
		.stepwise = {
			INGENIC_VENC_MIN_W, INGENIC_VENC_MAX_W, 16,
                           ...
                           struct v4l2_capability *cap)
{
	...
	/*
	 * driver version bit
	 * bit0~bit7	--> JPEG_Encoder
	 * bit8~bit15	--> JPEG_Decoder
	 * bit16~bit23	--> H264_Encoder
	 * bit24~bit31  --> reserved
	 * */
	cap->version = (H264_ENCODER_VERSION << 16) | (JPEG_DECODER_VERSION << 8) | JPEG_ENCODER_VERSION;
...
{
	struct ingenic_venc_ctx *ctx = fh_to_ctx(priv);
	int ret = v4l2_m2m_dqbuf(file, ctx->m2m_ctx, buf);
	if(ctx->codec_id == CODEC_ID_JPGD && buf->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE){
		buf->reserved = 0;
		buf->reserved |= ctx->jpgd_ctx.p.width << 16;
		buf->reserved |= ctx->jpgd_ctx.p.height;
	}
	return ret;
