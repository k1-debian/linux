...
static int h264_select_output_frame(H264Context *h)
{
	...
	int max_frame_buffers = h->max_frame_buffers - 1;
	...
	for (i = 0; 1; i++) {
		if (i == max_frame_buffers || cur->poc < h->last_pocs[i]) {
			if (i) {
	...
	out_of_order = max_frame_buffers - i;
	if (cur->f->pict_type == AV_PICTURE_TYPE_B
	    || (h->last_pocs[max_frame_buffers - 2] > INT_MIN && h->last_pocs[max_frame_buffers - 1] - h->last_pocs[max_frame_buffers - 2] > 2)) {
		out_of_order = FFMAX(out_of_order, 1);
	}

	if (out_of_order == max_frame_buffers) {
		//av_log(h->avctx, AV_LOG_VERBOSE, "Invalid POC %d<%d\n", cur->poc, h->last_pocs[0]);
		pr_err("Invalid POC %d<%d\n", cur->poc, h->last_pocs[0]);
		for (i = 1; i < max_frame_buffers; i++) {
			h->last_pocs[i] = INT_MIN;
		}
		h->last_pocs[0] = cur->poc;
		cur->mmco_reset = 1;
	} else if (h->avctx->has_b_frames < out_of_order && !sps->bitstream_restriction_flag) {
		pr_err("Increasing reorder buffer to %d\n", out_of_order);
	...
	av_assert0(pics <= max_frame_buffers);
...
void dump_picture(H264Picture *pic)
{
	...
	printf("f->buf[0]: %p\n", f->buf[0]);
	printf("f->buf[1]: %p\n", f->buf[1]);
	printf("f->buf[0]->buffer_pa: %x\n", f->buf[0]->buffer_pa);
	printf("f->buf[1]->buffer_pa: %x\n", f->buf[1]->buffer_pa);
...
static int decode_slice(struct AVCodecContext *avctx, void *arg)
{
    ...
    */
	...
	if (st_h264->cabac) {
		...
		src = (char *)sl->gb.buffer;
		dst = (char *)sl->gb.buffer;
...
#define ROUND_UP(x, align) (((int) (x) + (align - 1)) & ~(align - 1))
	...
	if (h->format == VPU_FORMAT_NV12 || h->format == VPU_FORMAT_NV21) {
		...
		st_h264->frm_y_stride = ROUND_UP(h->width, 16);
	...
	/*作废cache，预留内存复用的话，cache会有数据*/
	av_buffer_sync(&h->devmem_ctx, cur_pic->dec_result_y, DMA_TO_DEVICE);
	av_buffer_sync(&h->devmem_ctx, cur_pic->dec_result_uv, DMA_TO_DEVICE);
