...
static int min_mempool_size = 1024 * 1024 * 1024;
static int max_mempool_size = 0;
extern int dma_coherent_mem_available(struct device *dev);
static int devmem_alloc(devmem_ctx_t *ctx, devmem_info_t *di)
{
	...
	int pool = dma_coherent_mem_available(ctx->dev);
	if (pool > max_mempool_size) {
		max_mempool_size = pool;
		pr_err("max available: %d\n", pool);
	}
	if (pool < min_mempool_size) {
		pr_err("min available: %d\n", pool);
		min_mempool_size = pool;
	}
...
AVBuffer *av_buffer_alloc(devmem_ctx_t *ctx, unsigned int size)
{
	...
	buf->buffer = (void *)buf->di.userptr;
