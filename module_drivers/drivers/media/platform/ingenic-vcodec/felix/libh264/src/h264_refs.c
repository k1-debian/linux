                          ...
                          H264Picture *const *in, int len, int is_long, int sel)
{
	...
	while (i[0] < len || i[1] < len) {
		...
		/*直到 创建的个数大于len? 或者in[]picture存在并且 两个的帧类型相同?*/
...
static void h264_initialise_ref_list(H264Context *h, H264SliceContext *sl)
{
	...
	if (sl->slice_type_nos == AV_PICTURE_TYPE_B) {
		 ...
		 * 两个链表
		 * list0 是前向预测
		 * list1 是后向预测 只有B帧才会有list1。
		 *
		 * 根据pic_order_count来计算???
		 *
		 * */
		...
		/* 构建默认的 ref_list[0] 的短参考帧*/
		/*ref_list 是最终经过排序的参考列表, 短期参考帧和长期参考帧. */
		/* 这里h->short_ref_count可能为0，比如当前slice是IDR帧, 或者mmco_reset 要求复位参考列表时.
		 *
		 * 这里的short_ref与long_ref对应的是截至到上一帧，被用于标记的参考帧内容.
		 * 这里主要是要将short_ref放入到ref_list当中. 为什么不能直接放入呢？？？？？？
		 *
		 * */
		...
		/*如果构建的长度大于解析出来的ref_count， 将len到ref_count的内容清零：====》 不要了吗???*/
		/*参考列表中剩余的信息清零.*/
...
static void print_short_term(const H264Context *h)
{
	...
	if (h->avctx->debug & FF_DEBUG_MMCO) {
		...
		for (i = 0; i < h->short_ref_count; i++) {
			...
			av_log(h->avctx, AV_LOG_DEBUG, "%u fn:%d poc:%d %p\n",
...
static int pic_num_extract(const H264Context *h, int pic_num, int *structure)
{
	*structure = h->picture_structure;
	...
	/* 提取的方法是
	 * 如果当前图片为场图片，如果pic_num为偶数 ---> 修改类型为 PICT_FIELD_XXX??
	 *
	 *
	 * */
	if (FIELD_PICTURE(h)) {
...
int ff_h264_build_ref_list(H264Context *h, H264SliceContext *sl)
{
	...
	for (list = 0; list < sl->list_count; list++) {
		int pred = sl->curr_pic_num;  // framenum

		/* 根据slice_header里的list reordering 语法，重排序??*/
		for (index = 0; index < sl->nb_ref_modifications[list]; index++) {
			/*循环迭代，指导modification_of_pic_nums_idc == 3*/
			...
			switch (modification_of_pic_nums_idc) {
				...
				case 1: {
					...
					/* 指明需要重排序的图像. round(sl->max_pic_num) */
					pred &= sl->max_pic_num - 1;

					/*根据修改后的pred提取其frame_num， 帧号.*/
					...
					/*如果i >=0, 说明在short_ref中存在frame_num了?, 修改参考帧的pic_id为计算出来的pred.???? */

					/*那么 ref->frame_num 什么时候修改呢？？？？？？？ */

					/* frame_num 和 pic_id 的关系.........??*/

					/* 这里的pic_id 为什么像是计算出来的poc?
					 * 更新参考列表的pic_id????
					 * */
					if (i >= 0) {
						ref->pic_id = pred;
			...
			if (i < 0) {
				...
				/*遍历 每个list 的ref_count, ref_count是通过码流解析出来的....*/

				for (i = index; i + 1 < sl->ref_count[list]; i++) {

					/*如果某个ref_list的parent存在， parent是个什么鬼？干什么用的？？*/

					/* parent 应该时当前Ref所属的Pic？？？？*/
	...
	/*遍历 ref_list， 有效性和default ref_list。？？？*/
	for (list = 0; list < sl->list_count; list++) {
...
#if 0
	/*使用short_ref当中的第一帧作为错误恢复的图片. 先不处理. */
...
int ff_h264_execute_ref_pic_marking(H264Context *h)
{
	...
	/* 要么使用滑动窗口标记，要么使用上面解码出来的mmco进行标记. */
	...
	if (!current_ref_assigned) {
		...
		if (h->short_ref_count && h->short_ref[0] == h->cur_pic_ptr) {
			...
			/*整体将short_ref往后移动一个单元, 将当前图像放入short_ref第一位*/
			if (h->short_ref_count)
