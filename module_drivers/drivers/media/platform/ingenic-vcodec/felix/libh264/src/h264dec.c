...
int h264_enqueue_frame(H264Context *h, AVFrame *f)
{
	...
	if (f->queued == 1) {
		printk("warning f: already queued: %x\n", (uint32_t)f);
	}
...
void dump_dpb_buffer(H264Context *h)
{
	AVFrame *f = NULL;
	int i;
	...
	for (i = 0; i < H264_MAX_PICTURE_COUNT; i++) {
		f = h->DPB[i].f;
		printk("[%d]: %x, queued: %d\n", i, (uint32_t)f, f != NULL ? f->queued : 0);
	}
}
...
static int h264_init_context(AVCodecContext *avctx, H264Context *h)
{
	...
	h->max_frame_buffers = MAX_DELAYED_PIC_COUNT;
	...
	h->vpu_ctx.desc_va = (unsigned int)h->dma_desc->buffer;
...
__attribute__((__unused__)) static int get_last_needed_nal(H264Context *h)
{
	int nals_needed = 0;
	int first_slice = 0;
	int i, ret;
...
__attribute__((__unused__)) static int is_extra(const uint8_t *buf, int buf_size)
{
	int cnt = buf[5] & 0x1f;
...
#define DECODE_WITH_CORRUPT_FRAMES 1
#if DECODE_WITH_CORRUPT_FRAMES
	if (h->mb_y < h->mb_height || !h->mb_height) {
		h->current_slice = 0;
	} else if ((ret = ff_h264_field_end(h, &h->slice_ctx[0], 0)) < 0) {
		return ret;
	}
