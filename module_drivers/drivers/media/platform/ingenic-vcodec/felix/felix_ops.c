...
#define INGENIC_VDEC_MIN_W  32U
#define INGENIC_VDEC_MIN_H  32U
...
/*
 felix
      price_size = [H264dec (Macroblock, MB) + other size]

      OUT_size = ((w * h * 3 /2)/2 + 128) * max_out_frame_num
         [default M=2, test is working ok]

      CAP_size = (align(w, 128) * h * 3 /2) * max_frame_num
         [default N, depend on available memory space]

   N and M cat be setted in DTS through property:
      ingenic,max_out_frame_buffers , ingenic,max_frame_buffers
 */
#define MAX_SUPPORT_FRAME_BUFFERS     5
#define MAX_SUPPORT_OUT_FRAME_BUFFERS 3

unsigned int max_frame_buffers = MAX_SUPPORT_FRAME_BUFFERS;
module_param(max_frame_buffers, uint, S_IRUGO | S_IWUSR);

unsigned int max_out_frame_buffers = MAX_SUPPORT_OUT_FRAME_BUFFERS;
module_param(max_out_frame_buffers, uint, S_IRUGO | S_IWUSR);
...
static const struct ingenic_vcodec_framesizes ingenic_vcodec_framesizes[] = {
	{
		.fourcc = V4L2_PIX_FMT_H264,
		.stepwise = {
			INGENIC_VDEC_MIN_W, INGENIC_VDEC_MAX_W, 2,
                            ...
                            struct v4l2_format *f)
{
	...
	q_data->coded_width = ALIGN(f->fmt.pix_mp.width, 16);
	...
	h264_set_max_frame_buffers(ctx->h, max_frame_buffers);
	return ret;
}
...
/*  v4l2 userptr 模式下若是传进去的地址，与上次传入的地址和大小一样时，是不会重新去获取对应的物理地址的。
    但是对于应用层而言，映射的地址相同但是物理地址是有不一样的时候，所以这里dequeue的时候，
    vb2_buffer 相关数据拷贝到v4l2_buffer 之后，将length 置0，其会在videobuf2-core.c 的__prepare_userptr 函数中
    重新获取该传入对应的物理地址
*/
static void reset_userptr_vb2_buffer(struct ingenic_vdec_ctx *ctx, struct v4l2_buffer *buf)
{
	if (buf->type ==  V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE && buf->memory == VB2_MEMORY_USERPTR) {
		struct vb2_queue *vq = v4l2_m2m_get_vq(ctx->m2m_ctx, buf->type);
		struct vb2_buffer *vb_buf = vq->bufs[buf->index];

		int i;
		for (i = 0; i < vb_buf->num_planes; i++) {
			if (vb_buf->planes[i].mem_priv) {
				vb_buf->planes[i].length = 0;
			}
		}
	}
...
static int vidioc_dqbuf(struct file *file, void *priv, struct v4l2_buffer *buf)
{
	...
	reset_userptr_vb2_buffer(ctx, buf);
                              ...
                              struct v4l2_selection *s)
{
	...
	switch (s->target) {
		case V4L2_SEL_TGT_CROP:
			...
			s->r.left   = h->crop_right;
			s->r.top    = h->crop_bottom;
			s->r.width  = h->width;
			s->r.height = h->height;
			break;
...
void dump_vb2_on_queue(struct v4l2_m2m_queue_ctx *q_ctx)
{
	unsigned long flags;

	spin_lock_irqsave(&q_ctx->rdy_spinlock, flags);
	printk(" ---> [start], num_rdy: %d\n", q_ctx->num_rdy);
	if (list_empty(&q_ctx->rdy_queue)) {
		spin_unlock_irqrestore(&q_ctx->rdy_spinlock, flags);
		return;
	}

	struct v4l2_m2m_buffer *tmp;

	int list_index = 0;
	list_for_each_entry(tmp, &q_ctx->rdy_queue, list) {
		struct vb2_buffer *vb2 = &tmp->vb.vb2_buf;
		printk("[%d]: %x, vb2->state: %d\n", list_index++, (uint32_t)vb2, vb2->state);
	}
	printk(" ---> [end]\n");
	spin_unlock_irqrestore(&q_ctx->rdy_spinlock, flags);
}
...
static void ingenic_vdec_worker(struct work_struct *work)
{
...
#if 0
	{
		dump_vb2_on_queue(&ctx->m2m_ctx->cap_q_ctx);
	}
#endif
	...
	if (dst_buf && dst_buf->vb2_buf.state == VB2_BUF_STATE_ACTIVE) {
		...
		printk("----worker start with no frame buffer queued.\n");
		if (dst_buf) {
			printk("----dst_buf->vb2_buf.state: %d\n", dst_buf->vb2_buf.state);
		}
	}
	...
	if (got_frame) {
		...
		do {
			...
			if (f) {
				...
				if (done_buf->vb.vb2_buf.state != VB2_BUF_STATE_ACTIVE) {
					printk("-------------error buffer state: %d\n", done_buf->vb.vb2_buf.state);
				}
				if (done_buf->used == true) {
					v4l2_m2m_buf_done(&done_buf->vb, VB2_BUF_STATE_DONE);
					done_buf->used = false;
				} else {
					printk("---------- libh264 still used released buffers???\n");
				}
	...
	if ((decode_error) || (eos)) {
		...
		while ((f = h264_dequeue_frame(h))) {
			...
			//printk("-----buf_done from done list: %x, &err_buf->vb.vb2_buf: state %d\n", &err_buf->vb.vb2_buf, err_buf->vb.vb2_buf.state);
			vb2_set_plane_payload(&err_buf->vb.vb2_buf, 0, 0);
			vb2_set_plane_payload(&err_buf->vb.vb2_buf, 1, 0);

			err_buf->used = false;

			if (err_buf->vb.vb2_buf.state == VB2_BUF_STATE_ACTIVE) {
				v4l2_m2m_buf_done(&err_buf->vb, VB2_BUF_STATE_ERROR);
			} else {
				printk("error buf state why? retrain doned frame: %x, &err_buf->vb.vb2_buf: state %d\n", (uint32_t)&err_buf->vb.vb2_buf, err_buf->vb.vb2_buf.state);
			}
		}
		...
		while ((f = h264_get_queued_frame(h))) {
			struct ingenic_vcodec_buf *err_buf = av_frame_to_vcode_buf(f);
			//printk("-----retrain queued frame: %x, &err_buf->vb.vb2_buf: state %d\n", &err_buf->vb.vb2_buf, err_buf->vb.vb2_buf.state);
			vb2_set_plane_payload(&err_buf->vb.vb2_buf, 0, 0);
			vb2_set_plane_payload(&err_buf->vb.vb2_buf, 1, 0);
...
#if 1
			if (err_buf->vb.vb2_buf.state != VB2_BUF_STATE_ACTIVE) {
				printk("error buf state why? retrain queued frame: %x, &err_buf->vb.vb2_buf: state %d\n", (uint32_t)&err_buf->vb.vb2_buf, err_buf->vb.vb2_buf.state);
			}
#endif
			if (err_buf->used == true) {
				v4l2_m2m_buf_done(&err_buf->vb, VB2_BUF_STATE_ERROR);
				err_buf->used = false;
			} else {
				printk("why libh264 still hold buffer not used ????\n");
			}
		}
                                     ...
                                     struct device *alloc_ctxs[])
{
	...
	if (vq->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		if (*nbuffers > max_out_frame_buffers) {
			*nbuffers = max_out_frame_buffers;
		}
	} else {
...
static void vb2ops_vcodec_buf_queue(struct vb2_buffer *vb)
{
	...
	struct ingenic_vcodec_buf *buf = NULL;
	...
	if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		buf = container_of(vb2_v4l2, struct ingenic_vcodec_buf, vb);
		if (!buf->used) {
...
static void vb2ops_vcodec_buf_finish(struct vb2_buffer *vb)
{
	...
	struct vb2_v4l2_buffer *vb2_v4l2 = NULL;
	struct ingenic_vcodec_buf *buf = NULL;
	struct vb2_v4l2_buffer *tmp = NULL;
	...
	if (vb->vb2_queue->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE) {
		...
		tmp = v4l2_m2m_dst_buf_remove_by_idx(ctx->m2m_ctx, vb->index);
		if (tmp) {
			printk("====> removed buffer from rdy_queue: tmp: %x\n", (uint32_t)tmp);
		}
