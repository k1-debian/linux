...
static int _i2d_vidioc_expbuf(struct file *file, void *prv,
...
static void device_run(void *prv)
{
	...
	// wait process done
	ret = wait_for_completion_timeout(&ctx->ctx_done, msecs_to_jiffies(3000));
	if (ret < 0) {
		printk("wait done i2d error !!!!!\r\n");
	} else if (ret == 0) {
		printk("wait i2d done time out !!!!\r\n");
	} else {
		dev->cur_ctx = NULL;
	}

	mutex_unlock(&dev->i2d_mutex);
}

#define I2D_TIMEOUT 3000
...
static const struct v4l2_ioctl_ops i2d_ioctl_ops = {
	...
	.vidioc_expbuf          = _i2d_vidioc_expbuf,
...
static int ingenic_i2d_probe(struct platform_device *pdev)
{
	...
	ret = of_reserved_mem_device_init(i2d_dev->dev);
	if (ret) {
		dev_err(i2d_dev->dev, "Could not get reserved memory\n");
		goto release_v4l2_m2m;
	}

	return 0;

release_v4l2_m2m:
	v4l2_m2m_release(i2d_dev->m2m_dev);
unreg_video_dev:
