...
struct cim_fmt_lut cim_formats[] = {
	/*MONO */
	{
	...
	{
		.code = INGENIC_MEDIA_BUS_FMT_RBG565_2X8_LE,
		.fmt = {
			.name     = "RBG565, RBGP",
	...
	{
		.code = INGENIC_MEDIA_BUS_FMT_BRG565_2X8_LE,
		.fmt = {
			.name     = "BRG565, BRGP",
	...
	{
		.code = INGENIC_MEDIA_BUS_FMT_GRB565_2X8_LE,
		.fmt = {
			.name     = "GRB565, GRBP",
	...
	{
		.code = INGENIC_MEDIA_BUS_FMT_GBR565_2X8_LE,
		.fmt = {
			.name     = "GBR565, GBRP",
...
static int ingenic_cim_start(struct ingenic_camera_dev *pcdev);
...
static int get_asd_index(struct v4l2_async_connection *asd)
{
...
{
	const struct cim_fmt_lut *fmt = NULL;
	int i;
	for (i = 0; i < ARRAY_SIZE(cim_formats); i++) {
		fmt = &cim_formats[i];
		if (fourcc == fmt->fmt.fourcc) {
			return fmt->code;
		}
	}
	return 0;
...
struct cim_fmt_xlate *ingenic_cim_xlate_by_fourcc(struct cim_video_ctx *ctx, unsigned int fourcc)
{
	unsigned int i;

	for (i = 0; i < ctx->num_user_fmts; i++)
		if (ctx->user_fmts[i].fmt->fourcc == fourcc) {
			return ctx->user_fmts + i;
		}
	return NULL;
}
...
static int ingenic_camera_wakeup(struct ingenic_camera_dev *pcdev)
{
	...
	if (vbuf->vb2_buf.index != id) {
		struct ingenic_buffer *active_buf = NULL;
		int i = 0, active_num = 0;
		if (vbuf->vb2_buf.index < id) {
			active_num = id - vbuf->vb2_buf.index;
		} else {
			active_num = pcdev->buf_cnt - vbuf->vb2_buf.index + id;
		}

		for (i = 0; i < active_num; i++) {
			active_buf = list_entry(pcdev->video_buffer_list.next, struct ingenic_buffer, list);

			list_del_init(&active_buf->list);
			active_buf->vb.vb2_buf.timestamp = ktime_get_ns();
			active_buf->vb.sequence = pcdev->sequence++;
...
out:
	...
	if (cim_stopped == 1) {
		...
		pcdev->active = list_first_entry_or_null(&pcdev->video_buffer_list, struct ingenic_buffer, list);
		if (pcdev->active == NULL) {
			dev_err(pcdev->dev, "CIM is working while buffer list is empty!\n");
		}
	}
        ...
        struct v4l2_fmtdesc *f)
{
	...
	if (f->index >= ctx->num_user_fmts) {
		return -EINVAL;
	}

	f->pixelformat = ctx->user_fmts[f->index].fmt->fourcc;
...
static int __ingenic_camera_try_fmt(struct cim_video_ctx *ctx, struct v4l2_format *f)
{
	...
	int depth = 0;
	int ret = 0;
	struct cim_fmt_xlate *xlate;

	xlate = ingenic_cim_xlate_by_fourcc(ctx, pix->pixelformat);
	if (!xlate) {
		return -EINVAL;
	}
	ctx->current_fmt = xlate;
                                        ...
                                        struct v4l2_format *f)
{
	...
	ctx->sensor_info = (struct sensor_info *) * (unsigned int *)format.format.reserved;
...
static int ingenic_camera_g_input(struct file *file, void *fh, unsigned int *i)
{
	*i = 0;
	return 0;
}
...
/* Input handling */
static int ingenic_camera_enum_input(struct file *file, void *fh,
...
static const struct v4l2_ioctl_ops cim_video_ioctl_ops = {
	...
	.vidioc_enum_input      = ingenic_camera_enum_input,
	.vidioc_s_input         = ingenic_camera_s_input,
	.vidioc_g_input         = ingenic_camera_g_input,
...
static int ingenic_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	if (casd->bus_type == V4L2_MBUS_CSI2_DPHY) {
		if (ctx->sensor_info) {
			mipi_clk = ctx->sensor_info->mipi_clk;
		} else {
			printk("[Warning], ingenic cim sensor_info is invalid **** check ****\n");
		}
		ret = csi_phy_start(version, casd->mipi_csi2.num_data_lanes, mipi_clk);
		if (ret < 0) {
...
static void ingenic_camera_activate(struct ingenic_camera_dev *pcdev)
{
	...
	if (!IS_ERR_OR_NULL(pcdev->clk)) {
		ret = clk_prepare_enable(pcdev->clk);
	}
	if (!IS_ERR_OR_NULL(pcdev->mipi_clk)) {
...
struct cim_video_format *ingenic_cim_mbus_get_fmtdesc(u32 code)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(cim_formats); i++)
		if (cim_formats[i].code == code) {
			return &cim_formats[i].fmt;
		}

	return NULL;
}

int ingenic_camera_add_xlate(struct cim_video_ctx *ctx,
                             struct cim_video_format *fmt, int *num_fmts, u32 code)
{
	if (ctx->user_fmts) {
		ctx->user_fmts[*num_fmts].fmt   = fmt;
		ctx->user_fmts[*num_fmts].code  = code;
		(*num_fmts)++;
		ctx->num_user_fmts = *num_fmts;
	}
                                   ...
                                   struct v4l2_async_connection *asd)
{
	...
	int i = 0;
	int list_fmt[30] = {0};
	struct cim_video_format *fmt = NULL;
	struct v4l2_subdev_mbus_code_enum code = {
		.which = V4L2_SUBDEV_FORMAT_ACTIVE,
	};
	int raw_fmts = 0, fmts = 0, num_fmts = 0;
	...
	ctx->user_fmts = vmalloc(fmts * sizeof(struct cim_fmt_xlate));
	if (!ctx->user_fmts) {
		ret = -ENOMEM;
		goto err_ctx_formats_alloc;
	}
	while (i--) {
		if (pcdev->cim_priv->version_num == 2) {
			switch (list_fmt[i]) {
				...
				case MEDIA_BUS_FMT_SBGGR10_1X10:
				case MEDIA_BUS_FMT_SBGGR12_1X12:
					/*8/10/12 bit raw RGB -> 8 bit raw RGB*/
					fmt = ingenic_cim_mbus_get_fmtdesc(MEDIA_BUS_FMT_SBGGR8_1X8);
					ingenic_camera_add_xlate(ctx, fmt, &num_fmts, list_fmt[i]);
					/*8/10/12 bit raw RGB -> 8 bit y*/
					fmt = ingenic_cim_mbus_get_fmtdesc(MEDIA_BUS_FMT_Y8_1X8);
					ingenic_camera_add_xlate(ctx, fmt, &num_fmts, list_fmt[i]);
					break;
				case MEDIA_BUS_FMT_SGBRG8_1X8:
				case MEDIA_BUS_FMT_SGBRG10_1X10:
				case MEDIA_BUS_FMT_SGBRG12_1X12:
					/*8/10/12 bit raw RGB -> 8 bit raw RGB*/
					fmt = ingenic_cim_mbus_get_fmtdesc(MEDIA_BUS_FMT_SGBRG8_1X8);
					ingenic_camera_add_xlate(ctx, fmt, &num_fmts, list_fmt[i]);
					/*8/10/12 bit raw RGB -> 8 bit y*/
					fmt = ingenic_cim_mbus_get_fmtdesc(MEDIA_BUS_FMT_Y8_1X8);
					ingenic_camera_add_xlate(ctx, fmt, &num_fmts, list_fmt[i]);
				...
				case MEDIA_BUS_FMT_SGRBG12_1X12:
					/*8/10/12 bit raw RGB -> 8 bit raw RGB*/
					fmt = ingenic_cim_mbus_get_fmtdesc(MEDIA_BUS_FMT_SGRBG8_1X8);
					ingenic_camera_add_xlate(ctx, fmt, &num_fmts, list_fmt[i]);
					/*8/10/12 bit raw RGB -> 8 bit y*/
					fmt = ingenic_cim_mbus_get_fmtdesc(MEDIA_BUS_FMT_Y8_1X8);
					ingenic_camera_add_xlate(ctx, fmt, &num_fmts, list_fmt[i]);
				...
				case MEDIA_BUS_FMT_SRGGB12_1X12:
					/*8/10/12 bit raw RGB -> 8 bit raw RGB*/
					fmt = ingenic_cim_mbus_get_fmtdesc(MEDIA_BUS_FMT_SRGGB8_1X8);
					ingenic_camera_add_xlate(ctx, fmt, &num_fmts, list_fmt[i]);
					/*8/10/12 bit raw RGB -> 8 bit y*/
					fmt = ingenic_cim_mbus_get_fmtdesc(MEDIA_BUS_FMT_Y8_1X8);
					ingenic_camera_add_xlate(ctx, fmt, &num_fmts, list_fmt[i]);
				...
				default:
					/*v2 pass through*/
					fmt = ingenic_cim_mbus_get_fmtdesc(list_fmt[i]);
					ingenic_camera_add_xlate(ctx, fmt, &num_fmts, list_fmt[i]);
			}
		} else {
			/*v1 pass through*/
...
static int ingenic_cim_phrase_remote_endpoint(struct ingenic_camera_dev *pcdev)
{
	...
	struct cim_async_subdev *casd = NULL;
	...
	while ((node = of_graph_get_next_endpoint(pcdev->dev->of_node, node)) && pcdev->asd_num < MAX_ASYNC_SUBDEVS) {
		...
		fwnode = fwnode_graph_get_remote_port_parent(of_fwnode_handle(node));
		of_node_put(node);

		casd = v4l2_async_nf_add_fwnode(&pcdev->notifier, fwnode, struct cim_async_subdev);
		if (IS_ERR(casd)) {
		...
		casd->bus_type = pcdev->vep[pcdev->asd_num].bus_type;
		casd->parallel = pcdev->vep[pcdev->asd_num].bus.parallel;
		casd->mipi_csi2 = pcdev->vep[pcdev->asd_num].bus.mipi_csi2;
		casd->index = pcdev->asd_num;
		pcdev->casd[pcdev->asd_num] = casd;
...
static const struct of_device_id ingenic_camera_of_match[] = {
	...
	{ .compatible = "ingenic,x2600-cim", .data = (struct cim_private *) &version2_pri },
	{ },
};
...
static int ingenic_camera_probe(struct platform_device *pdev)
{
	...
	ret = v4l2_async_nf_register(&pcdev->notifier);
	if (ret) {
		printk("v4l2_async_nf_register failed %d\n", ret);
