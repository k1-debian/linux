...
static int vic_subdev_core_init(struct v4l2_subdev *sd, u32 sensor_id)
{
	return 0;
}
...
static int vic_cfg_stream_dvp(struct vic_device *vic)
{
	...
	if (dvp_cfg->dvp_mode == SENSOR_DVP_HSYNC_MODE) {
		input_cfg |= (1 << 15);    /*hsync mode*/
	}
	...
	if (input_fmt->format.code == MEDIA_BUS_FMT_YUYV8_2X8 || input_fmt->format.code == MEDIA_BUS_FMT_YVYU8_2X8
	    || input_fmt->format.code == MEDIA_BUS_FMT_UYVY8_2X8 || input_fmt->format.code == MEDIA_BUS_FMT_VYUY8_2X8) {
		vic_reg_writel(vic, VIC_IN_HOR_PARA0, input_fmt->format.width * 2 + (dvp_cfg->hfb_num << 16) /*hblank*/);
	} else {
		vic_reg_writel(vic, VIC_IN_HOR_PARA0, input_fmt->format.width + (dvp_cfg->hfb_num << 16) /*hblank*/);
	}
	vic_reg_writel(vic, VIC_IN_VER_PARA3, dvp_cfg->in_ver_para3 /*vblank*/);
	// vc control
...
static int vic_cfg_stream_bt1120(struct vic_device *vic)
{
	struct v4l2_subdev_format *input_fmt = &vic->formats[VIC_PAD_SINK];
	struct v4l2_mbus_config_parallel *parallel = ispcam_get_bus_parallel(vic->ispcam);
	struct bt_cfg *bt_cfg = &vic->sensor_info->bt_cfg;
	unsigned int input_cfg = 0;
	...
	dev_dbg(vic->dev, "parallel->bus_width: %d\n", parallel->bus_width);
	dev_dbg(vic->dev, "parallel->data_shift: %d\n", parallel->data_shift);
	vic_reg_writel(vic, VIC_INTF_TYPE, INTF_TYPE_BT1120);
	...
	if (parallel->bus_width == 16) {
		input_cfg |= BT_INTF_WIDE;
		input_cfg |= DVP_YUV422_16BIT;
	} else {
		input_cfg |= DVP_YUV422_8BIT;
	}

	if (bt_cfg->interlace_en) {
		input_cfg |= INTERLACE_EN;
	}
	if (bt_cfg->bt_sav_eav == SAV_BF_EAV) {
		input_cfg |= BT_LINE_MODE;
	}
	...
	if (enable) {
		...
		switch (input_fmt->format.code) {
			case MEDIA_BUS_FMT_Y8_1X8 :
				...
				width = input_fmt->format.width / 2;
				break;
			/* TODO: NV21/NV12 */
			default:
		...
		if (ispvideo->bypass == 1) {
			...
			regval = 0;
			/*TODO: fmts.*/
			switch (ispvideo->current_format->fourcc) {
				...
				case V4L2_PIX_FMT_NV12 :
					regval |= 7 << 0; /* mode 7: NV12 different from spec*/
					y_stride = width;
					uv_stride = width;
					break;
				case V4L2_PIX_FMT_NV21 :
					regval |= 6 << 0; /* mode 6: NV12 different from spec*/
					y_stride = width;
					uv_stride = width;
					break;
			...
			vic_reg_writel(vic, VIC_DMA_Y_STRID, y_stride);
			vic_reg_writel(vic, VIC_DMA_UV_STRID, uv_stride);

			if (vic->enabled == 0) {
				vic_reg_writel(vic, VIC_CONTROL_TIZIANO_ROUTE, 0x4444);
			}
		...
		} else if (isd->bus_type == V4L2_MBUS_BT656) {
			ret = vic_cfg_stream_bt1120(vic);
			mux_out |= INPUT_MXU_DVP_EN;
		}
...
dump_vic(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	p += sprintf(p, "VIC_CONTROL_INPUT_MUX_OUTROUTE          :0x%08x\n", vic_reg_readl(vic, VIC_CONTROL_INPUT_MUX_OUTROUTE));
...
ssize_t vic_dma_debug(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "snapraw", sizeof("snapraw") - 1)) {
		...
		if (vic->snap_vaddr) {
			...
			if (vic->sensor_info->wdr_en) {
				kernel_write(fd, vic->snap_vaddr + imagesize, imagesize, pos);
			}
			filp_close(fd, NULL);
		}
	}
