...
typedef enum {
	...
	IMPISP_FLIP_SENSOR_H_MODE,
	IMPISP_FLIP_SENSOR_V_MODE,
	IMPISP_FLIP_SENSOR_HV_MODE,
	IMPISP_FLIP_IHSV_MODE,
	IMPISP_FLIP_SHIV_MODE,
...
typedef struct {
	...
	tisp_pic_image_type_t osd_image_type; /**< osd picture type */
...
typedef struct {
	...
	tisp_argb_type_t osd_argb_type;                 /**< OSD argb type */
	TISP_OPS_MODE osd_pixel_alpha_disable;    /**< OSD pixel alpha disable function enable */
	tisp_osd_pic_attr_t pic;                        /**< OSD picture attribution */
} tisp_single_osd_attr_t;
...
typedef struct {
	...
	tisp_af_ldg_en_t ldg_en;
	tisp_af_ldg_info_t fir0;
	tisp_af_ldg_info_t fir1;
	tisp_af_ldg_info_t iir0;
	tisp_af_ldg_info_t iir1;
...
typedef struct {
	TISP_OPS_MODE enable;
	unsigned int dark;
	unsigned int middark;
	unsigned int midbright;
	unsigned int morebright;
	unsigned int bright;
	unsigned int target;
} tisp_defog_t;
...
typedef struct {
	int16_t strength;       /**< 畸变矫正的强度*/
	int16_t width;          /**< 图像宽度 */
	int16_t height;         /**< 图像高度 */
	int16_t center_w;       /**< 图像畸变水平光学中心 范围:[width/2-120, width/2+120] */
	int16_t center_h;       /**< 图像畸变垂直光学中心 范围:[height/2-120, height/2+120] */
} tisp_hldc_attr_t;
...
int tisp_s_antiflick(int vinum, tisp_anfiflicker_attr_t *attr, TISP_MODE_DN_E dn);
...
int tisp_s_osd_single_attr(int vinum, tisp_single_osd_attr_t *attr);
int tisp_g_osd_single_attr(int vinum, tisp_single_osd_attr_t *attr);
...
int32_t tisp_s_hldc_attr(int vinum, tisp_hldc_attr_t *attr);
int32_t tisp_g_hldc_attr(int vinum, tisp_hldc_attr_t *attr);
int32_t tisp_s_fixed_contrast_attr(int vinum, tisp_fixed_contrast_attr_t *attr);
int32_t tisp_lsc_hvflip(int vinum, int32_t width, int32_t height, int32_t flip_en, int32_t mirror_en);
...
int tisp_defog_set(int vinum, tisp_defog_t *attr, tisp_defog_t *stren_y);
void tisp_set_csc_clip2_uv(int vinum, tisp_uv_thr_t *thr_uv);
int tisp_set_hilightdepress(int vinum, int strength);
int tisp_get_hilightdepress(int vinum, int *strength);
#endif
