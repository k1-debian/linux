...
static int
isp_video_get_selection(struct file *file, void *fh, struct v4l2_selection *s)
{
	struct isp_video_device *ispvideo = video_drvdata(file);
	unsigned int pad;
...
static int
isp_video_set_selection(struct file *file, void *fh, struct v4l2_selection *s)
{
	struct isp_video_device *ispvideo = video_drvdata(file);
	unsigned int pad;
	struct v4l2_subdev *sd = isp_video_remote_subdev(ispvideo, &pad);
...
extern int isp_cpm_reset(struct isp_device *isp);
...
static void isp_video_stop_streaming(struct vb2_queue *q)
{
	...
	isp_cpm_reset(ispvideo->ispcam->isp);
