                           ...
                           struct v4l2_subdev_format *format)
{
	...
	/*def crop info.*/
	mscaler->sel[ch].r.left = 0;
	mscaler->sel[ch].r.top = 0;
	mscaler->sel[ch].r.width = format->format.width;
	mscaler->sel[ch].r.height = format->format.height;
...
static int mscaler_get_selection(struct v4l2_subdev *sd, struct v4l2_subdev_state *state, struct v4l2_subdev_selection *sel)
{
	...
	if (sel->target == V4L2_SEL_TGT_CROP || sel->target == V4L2_SEL_TGT_CROP_DEFAULT) {
		memcpy(&sel->r, &mscaler->sel[ch].r, sizeof(struct v4l2_rect));
...
static int mscaler_set_selection(struct v4l2_subdev *sd, struct v4l2_subdev_state *state, struct v4l2_subdev_selection *sel)
{
	...
	output_fmt = &mscaler->formats[sel->pad];
	if (sel->r.top + sel->r.height > output_fmt->format.height ||
	    sel->r.left + sel->r.width > output_fmt->format.width) {
		dev_warn(mscaler->dev, "selection out of range! please check the boundry.\n");
		return -EINVAL;
	}

	if (sel->target == V4L2_SEL_TGT_CROP) {
		memcpy(&mscaler->sel[ch].r, &sel->r, sizeof(struct v4l2_rect));
	} else {
		dev_warn(mscaler->dev, "unsupported selection target!\n");
		return -EINVAL;
	}
...
static int mscaler_stream_enable(struct mscaler_device *mscaler, int ch)
{
	...
	/*Crop info.*/
	mscaler_reg_writel(mscaler, CHx_CROP_OPOS(ch), (mscaler->sel[ch].r.left << 16) | mscaler->sel[ch].r.top);
	mscaler_reg_writel(mscaler, CHx_CROP_OSIZE(ch), (mscaler->sel[ch].r.width << 16) | mscaler->sel[ch].r.height);

	mscaler_reg_writel(mscaler, CHx_DMAOUT_Y_STRI(ch), mscaler->sel[ch].r.width);
	mscaler_reg_writel(mscaler, CHx_DMAOUT_UV_STRI(ch), mscaler->sel[ch].r.width);
