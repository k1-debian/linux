...
static int ingenic_isp_parse_dt(struct ispcam_device *ispcam)
{
	...
	struct isp_async_device *isd = NULL;
	...
	v4l2_async_nf_init(&ispcam->notifier, &ispcam->v4l2_dev);
	while ((node = of_graph_get_next_endpoint(dev->of_node, node))) {
		...
		if (fwnode->dev->driver) {
			...
			isd = v4l2_async_nf_add_fwnode(&ispcam->notifier, fwnode, struct isp_async_device);
			if (IS_ERR(isd)) {
			...
			isd->bus_type = ispcam->vep[ispcam->asd_num].bus_type;
			isd->parallel = ispcam->vep[ispcam->asd_num].bus.parallel;
			isd->mipi_csi2 = ispcam->vep[ispcam->asd_num].bus.mipi_csi2;
			isd->index = ispcam->asd_num;
			ispcam->isd[ispcam->asd_num] = isd;
			ispcam->asd_num ++;
	...
	v4l2_async_nf_cleanup(&ispcam->notifier);
	return -ENODEV;
...
static int ispcam_comp_bind(struct device *master)
{
	...
	ret = v4l2_async_nf_register(&ispcam->notifier);
	if (ret < 0) {
...
static int ingenic_isp_probe(struct platform_device *pdev)
{
	...
	ret = ingenic_isp_parse_dt(ispcam);
	if (ret < 0) {
		return ret;
	}
...
{
	struct ispcam_device *ispcam = platform_get_drvdata(pdev);
	v4l2_async_nf_cleanup(&ispcam->notifier);

	return 0;
}

struct ispcam_data ingenic_x2000_camera_data = {
	.chip_name = "x2000",
};

struct ispcam_data ingenic_m300_camera_data = {
	.chip_name = "m300",
};

static const struct of_device_id ingenic_isp_dt_match[] = {
	{ .compatible = "ingenic,x2000-isp-camera", .data = (void *) &ingenic_x2000_camera_data},
	{ .compatible = "ingenic,m300-isp-camera", .data = (void *) &ingenic_m300_camera_data},
	{ }
};
