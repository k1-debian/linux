...
static int ispcore_exp_auto_s_control(tisp_core_t *core)
{
	...
	if (isp->ctrls.exp_auto == V4L2_EXPOSURE_AUTO) {
		...
		dev_err(isp->dev, "the available range of auto expo is 0-1 0:auto 1:manual\n");
		return -EINVAL;
	}
...
static int ispcore_exp_abs_s_control(tisp_core_t *core)
{
	...
	if (ae_attr.manual_it) {
		if (isp->ctrls.exp_abs < 0) {
			dev_err(isp->dev, "the available range of exposure abs is greater than 0\n");
			ret = -EINVAL;
		} else {
...
static inline int ispcore_auto_gain_s_control(tisp_core_t *core)
{
	...
	if (isp->ctrls.auto_gain == 0) {
		...
		dev_err(isp->dev, "the available range of auto gain is 0-1 0:auto 1:manual\n");
		return -EINVAL;
	}
...
static inline int ispcore_gain_s_control(tisp_core_t *core)
{
	...
	if (ae_attr.manual_ag) {
		if (isp->ctrls.gain < 0) {
			dev_err(isp->dev, "the available range of gain is greater than 0\n");
			ret = -EINVAL;
		} else {
...
static inline int ispcore_auto_wb_s_control(tisp_core_t *core)
{
	...
	case 0:
		...
		break;
	default:
		dev_err(isp->dev, "the available range of auto_wb is 0-1 0:manual 1:auto\n");
		ret = -EINVAL;
		break;
	}
	return ret;
...
static inline int ispcore_auto_n_wb_s_control(tisp_core_t *core)
{
	...
	case V4L2_WHITE_BALANCE_FLUORESCENT_H:
		...
		ret = tisp_s_wb_attr(&core->core_tuning, *wb_attr);
		break;
	case 10:/*IMAGE_WHITE_BALANCE_CUSTOM*/
		wb_attr->tisp_wb_manual = 9;
		ret = tisp_s_wb_attr(&core->core_tuning, *wb_attr);
		ret = tisp_s_wb_frz_attr(&core->core_tuning, 1);
		break;
	default:
		dev_err(isp->dev, "the available range of auto_wb is 0-10, but 7 is invalid \n");
		ret = -EINVAL;
		break;
	}
	return ret;
...
static inline int apical_isp_day_or_night_s_ctrl(tisp_core_t *core)
{
	...
	if (dn != 0 && dn != 1) {
		dev_err(isp->dev, "the available range of daynight mode is 0-1, 0:day 1:night\n");
		return -EINVAL;
	}
...
static int apical_isp_roi_ae_s_ctrl(struct isp_video_device *ispvideo, tisp_core_t *core)
{
	...
	tisp_area_t attr = isp->ctrls.area;
	int ret = 0;
	...
	if (attr.top < 0 || attr.top > out_height || attr.bottom < 0 || attr.bottom > out_height
		...
		dev_err(isp->dev, "roi ae attr not available\n");
		return -EINVAL;
	} else {
		...
		ret = tisp_ae_area_set(&core->core_tuning, &attr);
		if (ret != 0) {
			printk("[ %s:%d ] set roi ae failed!!!\n", __func__, __LINE__);
		}
...
static int isp_video_s_ctrl_inited(struct isp_video_device *ispvideo, struct v4l2_control *a)
{
	struct ispcam_device    *ispcam = ispvideo->ispcam;
	struct isp_device   *isp    = ispcam->isp;
	tisp_core_t *core = &isp->core;
	...
	case IMAGE_TUNING_CID_DAY_OR_NIGHT:
		...
		ret = apical_isp_day_or_night_s_ctrl(core);
		break;
	case IMAGE_TUNING_CID_ROI_AE:
		ret = copy_from_user(&isp->ctrls.area, (const void __user *)a->value, sizeof(isp->ctrls.area));
		ret = apical_isp_roi_ae_s_ctrl(ispvideo, core);
		break;
	default:
		ret = -EINVAL;
		break;
...
static int isp_video_s_ctrl_noinited(struct isp_video_device *ispvideo, struct v4l2_control *a)
{
	struct ispcam_device    *ispcam = ispvideo->ispcam;
	struct isp_device   *isp    = ispcam->isp;
	int ret = 0;
	...
	case V4L2_CID_EXPOSURE_AUTO:
		if (a->value != 0 && a->value != 1) {
			dev_err(isp->dev, "the available range of auto exp is 0-1 0:auto 1:manual\n");
			return -EINVAL;
		}
		isp->ctrls.exp_auto = a->value;
	...
	case V4L2_CID_AUTO_N_PRESET_WHITE_BALANCE:
		if (a->value < 0 || a->value > 9) {
			dev_err(isp->dev, "the available range of auto_wb is 0-9 \n");
			ret = -EINVAL;
		}
		isp->ctrls.wb    = a->value;
	...
	case IMAGE_TUNING_CID_HILIGHTDEPRESS:
		...
		break;
	case V4L2_CID_POWER_LINE_FREQUENCY:
		if (a->value != 0 && a->value != 1 && a->value != 2) {
			dev_err(isp->dev, "the available range of antiflicker is 0-2 0:disable 1:50Hz 2:60Hz\n");
			return -EINVAL;
		}
		isp->ctrls.flicker = a->value;
...
int isp_video_s_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	...
	if (isp->core_inited) {
		ret = isp_video_s_ctrl_inited(ispvideo, a);
	} else {
		ret = isp_video_s_ctrl_noinited(ispvideo, a);
	}
	return ret;
}
