                              ...
                              struct v4l2_subdev_format *format)
{
	...
	remote = media_pad_remote_pad_unique(&csi->pads[CSI_PAD_SINK]);
                              ...
                              struct v4l2_subdev_format *format)
{
	...
	csi->sensor_info = (struct sensor_info *) * (unsigned int *)remote_subdev_fmt.format.reserved;
...
{
	unsigned int mask = (1 << width) - 1;
	unsigned int temp = readl((unsigned int *)(iobase + address));

	temp &= ~(mask << shift);
	temp |= (data & mask) << shift;
	writel(temp, (unsigned int *)(iobase + address));
	return 0;
}
...
{
	struct csi_device *csi = v4l2_get_subdevdata(sd);
	struct v4l2_mbus_config_mipi_csi2 *mipi_csi2 = ispcam_get_bus_mipi_csi2(csi->ispcam);
	unsigned short nlanes = mipi_csi2->num_data_lanes;
	unsigned int mipi_clk = csi->sensor_info->mipi_cfg.clk;
	...
	if (enable) {
		...
		if (precounter_data && precounter_clk) {
			dev_nr = csi->ispcam->dev_nr;

			if (dev_nr == 0) {
				reg = csi_phy_readl(csi, CSI_PHY_PRECOUNTER_IN_CLK);
				reg &= ~0xff;
				reg |= precounter_clk;
				csi_phy_writel(csi, CSI_PHY_PRECOUNTER_IN_CLK, reg);

				reg = csi_phy_readl(csi, CSI_PHY_PRECOUNTER_IN_DATA);
				if (nlanes > 2) {
					reg = precounter_data | (precounter_data << 8) | (precounter_data << 16) | (precounter_data << 24);
				} else {
					reg &= ~0xffff;
					reg = reg | precounter_data | (precounter_data << 8);
				}
				csi_phy_writel(csi, CSI_PHY_PRECOUNTER_IN_DATA, reg);
			} else if (dev_nr == 1) {
				reg = csi_phy_readl(csi, CSI_PHY_PRECOUNTER_IN_CLK);
				reg &= ~0xff00;
				reg |= (precounter_clk << 8);
				csi_phy_writel(csi, CSI_PHY_PRECOUNTER_IN_CLK, reg);

				reg = csi_phy_readl(csi, CSI_PHY_PRECOUNTER_IN_DATA);
				reg &= ~0xffff0000;
				reg = reg | (precounter_data << 16) | (precounter_data << 24);
				csi_phy_writel(csi, CSI_PHY_PRECOUNTER_IN_DATA, reg);
			}
		} else if (csi->clk_precounter && csi->data_precounter) {
...
dump_csi(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	p += sprintf(p, "CSI_PHY_PRECOUNTER_IN_CLK  :0x%08x\n",     csi_phy_readl(csi, CSI_PHY_PRECOUNTER_IN_CLK));
	p += sprintf(p, "CSI_PHY_PRECOUNTER_IN_DATA :0x%08x\n",     csi_phy_readl(csi, CSI_PHY_PRECOUNTER_IN_DATA));
