...
static void ingenic_cdbus_timer_handler(struct timer_list *t)
{
	struct jz_cdbus *cdbus = from_timer(cdbus, t, timer);
...
static int jz_cdbus_open(struct inode *inode, struct file *filp)
{
	...
	timer_setup(&cdbus->timer, ingenic_cdbus_timer_handler, 0);
