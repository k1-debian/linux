...
static ssize_t bl_dbgfs_rssi_read(struct file *file,
                                       char __user *user_buf,
                                       size_t count, loff_t *ppos)
{
    struct bl_sta *sta = NULL;
...
static ssize_t bl_dbgfs_rssi_write(struct file *file,
                                        const char __user *user_buf,
                                        size_t count, loff_t *ppos)
{
    struct bl_sta *sta = NULL;
...
DEBUGFS_READ_WRITE_FILE_OPS(rssi);
...
static void bl_rc_stat_work(struct work_struct *ws)
{
    ...
    if (bl_debugfs->dir_sta[sta_idx] == NULL) {
        ...
        file = debugfs_create_file("rssi", S_IRUSR | S_IWUSR, dir_sta, bl_hw,
                                   &bl_dbgfs_rssi_ops);
        if (IS_ERR_OR_NULL(file))
            goto error_after_dir;
...
static ssize_t bl_dbgfs_oppps_write(struct file *file,
                                      const char __user *user_buf,
                                      size_t count, loff_t *ppos)
{
    struct bl_hw *bl_hw = file->private_data;
    ...
    if (sscanf(buf, "ctw=%d", &ctw) > 0) {
        ...
        list_for_each_entry(bl_vif, &bl_hw->vifs, list) {
            if (BL_VIF_TYPE(bl_vif) == NL80211_IFTYPE_P2P_GO) {
                struct mm_set_p2p_oppps_cfm cfm;

                /* Forward request to the embedded and wait for confirmation */
                bl_send_p2p_oppps_req(bl_hw, bl_vif, (u8)ctw, &cfm);
...
static ssize_t bl_dbgfs_noa_write(struct file *file,
                                    const char __user *user_buf,
                                    size_t count, loff_t *ppos)
{
    struct bl_hw *bl_hw = file->private_data;
    ...
    if (sscanf(buf, "count=%d interval=%d duration=%d dyn=%d",
        ...
        list_for_each_entry(bl_vif, &bl_hw->vifs, list) {
            if (BL_VIF_TYPE(bl_vif) == NL80211_IFTYPE_P2P_GO) {
                ...
                /* Forward request to the embedded and wait for confirmation */
                bl_send_p2p_noa_req(bl_hw, bl_vif, (u8)noa_count, (u8)interval,
                                      (u8)duration, (dyn_noa > 0),  &cfm);
...
void bl_dbgfs_unregister(struct bl_hw *bl_hw)
{
    ...
    if(bl_debugfs->helper_work.func)
        flush_work(&bl_debugfs->helper_work);
#ifdef CONFIG_BL_FULLMAC
    if(bl_debugfs->rc_stat_work.func)
