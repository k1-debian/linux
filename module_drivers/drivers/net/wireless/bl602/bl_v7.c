...
int bl_device_init(struct sdio_func *func, struct bl_plat **bl_plat)
{
    ...
    *bl_plat = kzalloc(sizeof(struct bl_plat) + sizeof(struct bl_device), GFP_KERNEL);
...
#else
	sdio_writeb(func, 0, device->reg->host_int_rsr_reg, &ret);
	if (ret) {
	...
	(*bl_plat)->mp_regs_buf = kzalloc(device->reg->max_mp_regs + DMA_ALIGNMENT, GFP_KERNEL|GFP_DMA);
	(*bl_plat)->mp_regs = (u8 *)ALIGN_ADDR((*bl_plat)->mp_regs_buf, DMA_ALIGNMENT);
	if (!(*bl_plat)->mp_regs) {
