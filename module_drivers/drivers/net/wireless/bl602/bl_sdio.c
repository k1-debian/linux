...
int bl_get_wr_port(struct bl_hw *bl_hw, u32 *port)
{
	...
	if (!(wr_bitmap & bl_device->reg->data_port_mask)) {
		printk("no available port wr_bitmap=0x%x\n", wr_bitmap);
		return -EBUSY;
	}
	...
	if (wr_bitmap & (1 << bl_hw->plat->curr_wr_port)) {
		...
		*port = bl_hw->plat->curr_wr_port;
		...
		printk("no available port, w_bitmap=0x%x, mp_wr_bitmap=0x%x, curr_wr_port=0x%x\n", wr_bitmap, bl_hw->plat->mp_wr_bitmap, bl_hw->plat->curr_wr_port);
...
static int bl_io_write(struct bl_plat *card, u8* data, int len)
{
    ...
    BL_DBG("bl_io_write: wr_bitmap=0x%x, count %d\n", bitmap, count);
    ...
    bootheader_t *header, const u8 **data, int *len)
{
    ...
    BL_DBG("Get SEGMENT From BL606: destaddr: 0x%08x, len %d, rsv %08X, CRC32 %08X\n",
        segment->destaddr,
        segment->len,
        segment->rsv,
        segment->crc32
...
static int bl_sdio_download_firmware(struct bl_plat *card)
{
    ...
    char *fw_name = "wholeimg_if.bin";
    if (bl_mod_params.mp_mode) {
        fw_name = "wholeimg_if_mp.bin";		
        printk("to download mfg fw\n");
    }

    ret = request_firmware(&fw_helper, fw_name, &card->func->dev);
    if ((ret < 0) || !fw_helper) {
        printk("request_firmware failed, fw_name=%s, error code = %d", fw_name, ret);
        ret = -ENOENT;
        return ret;
    }
...
#ifdef CONFIG_BL_MP
    if (bl_mod_params.mp_mode){
        image = wholeimg_if_mp_bin;
        imagelen = wholeimg_if_mp_bin_len;	
        printk("to download mfg fw\n");
	} else {
#endif		
        image = wholeimg_if_bin;
        imagelen = wholeimg_if_bin_len;
#ifdef CONFIG_BL_MP
	}
#endif	
#endif
...
int bl_write_data_sync(struct bl_hw *bl_hw, u8 *buffer, u32 pkt_len, u32 port)
{
	...
	/* issue abort cmd52 command through 0x1  reset SDIO 0x8*/
	if(ret){
		sdio_f0_writeb(bl_hw->plat->func, 0x1, SDIO_CCCR_ABORT, &status);
	}
 ...
 * This function reads data from SDIO card register.
 */
int bl_read_reg_claim0(struct bl_hw *bl_hw, u32 reg, u8 *data)
{
	int ret = -1;
	u8 val;
...
/*
 * This function reads data from SDIO card fun0 register.
 */
int bl_read_fun0_reg(struct bl_hw *bl_hw, u32 reg, u8 *data)
{
	int ret = -1;
	u8 val;

	sdio_claim_host(bl_hw->plat->func);
	val = sdio_f0_readb(bl_hw->plat->func, reg, &ret);
	sdio_release_host(bl_hw->plat->func);
	*data = val;
