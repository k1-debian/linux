...
static inline bool is_non_blocking_msg(int id) {
    return ((id == MM_TIM_UPDATE_REQ) || (id == ME_RC_SET_RATE_REQ) ||
            (id == ME_TRAFFIC_IND_REQ) || (id == SM_EXTERNAL_AUTH_REQUIRED_RSP));
}
                         ...
                         int reqcfm, lmac_msg_id_t reqid, void *cfm)
{
    ...
    if (!test_bit(BL_DEV_STARTED, &bl_hw->drv_flags) &&
        ...
        reqid != ME_CHAN_CONFIG_CFM && reqid != MM_SET_RFTLV_CFM && reqid != MM_CAL_CFG_CFM) {
...
int bl_send_caldata(struct bl_hw *bl_hw)
{
    int    ret = 0;
    u16_l  rf_buf_len = 0;
    u8_l * rf_buf_addr = NULL;
    struct mm_set_rftlv_req * rftlv_req = NULL;

#ifdef CONFIG_BL_DNLD_FWBIN
    const struct firmware *fw_helper = NULL;
    ret = request_firmware(&fw_helper, "bl_caldata.bin", &bl_hw->plat->func->dev);
    if ((ret < 0) || !fw_helper) {
        printk("request bl_caldata failed, error code = %d", ret);
        ret = -ENOENT;
        return ret;
    }
    rf_buf_addr = fw_helper->data;
    rf_buf_len = fw_helper->size;
#else
    rf_buf_addr = bl_caldata_bin;
    rf_buf_len = bl_caldata_bin_len;
#endif

    if(!rf_buf_len) {
        printk("no caldata bin array, skip\n");
        ret = -1;
        goto exit;
    }
    BL_DBG("rftlv buf_len %d, rftlv_buf_len=%d\n", sizeof(struct mm_set_rftlv_req), rf_buf_len);
    rftlv_req = bl_msg_zalloc(MM_SET_RFTLV_REQ, TASK_MM, DRV_TASK_ID,
                                sizeof(struct mm_set_rftlv_req));
    if (!rftlv_req) {
        ret = -1;
        goto exit;
    }

    if(bl_parse_caldata(rf_buf_addr, rf_buf_len, rftlv_req))
        goto exit;

    ret = bl_send_msg(bl_hw, rftlv_req, 1, MM_SET_RFTLV_CFM, NULL);
...
int bl_send_mp_test_msg(struct bl_hw *bl_hw, char *mp_cmd, char *mp_test_cfm, bool nonblock)
{
    ...
    msg_params = bl_msg_zalloc(MP_TEST_REQ, TASK_MP, DRV_TASK_ID, 2048);
    if (!msg_params)
        return -ENOMEM;

    msg_len = strlen(mp_cmd) + sizeof(struct sdio_msg) - 2;
    sdio_msg = (struct sdio_msg *)msg_params;
    sdio_msg->len_lsb = msg_len & 0xff;
    sdio_msg->len_msb = (msg_len >> 8) & 0xff;
    sdio_msg->type = SDIO_MSG_UART_DATA;
    memcpy(sdio_msg->payload, mp_cmd, strlen(mp_cmd));
    msg = container_of((void *)msg_params, struct lmac_msg, param);

    cmd = kzalloc(sizeof(struct bl_cmd), nonblock ? GFP_ATOMIC : GFP_KERNEL);
    cmd->result  = -EINTR;
    cmd->id      = msg->id;
    cmd->reqid   = MP_TEST_CFM;
    cmd->a2e_msg = msg;
...
{
    struct mm_rf_para_req *cal_cfg_req;

    BL_DBG(BL_FN_ENTRY_STR);

    /* Build the MM_CAL_CFG_REQ message */
    cal_cfg_req = bl_msg_zalloc(MM_CAL_CFG_REQ, TASK_MM, DRV_TASK_ID,
                                sizeof(struct mm_rf_para_req));
    if (!cal_cfg_req)
        return -ENOMEM;

    memcpy(cal_cfg_req->magic, "RFPA", strlen("RFPA"));
    if (capcode_valid) {
        cal_cfg_req->capcode = capcode;
        cal_cfg_req->capcode_valid = 0x5a;
        BL_DBG("cal_cfg_req->capcode is %d\n", cal_cfg_req->capcode);
    } else {
        cal_cfg_req->capcode_valid = 0x00;
    }
    if (pwr_oft_valid) {
        memcpy(cal_cfg_req->poweroffset, pwr_oft, sizeof(cal_cfg_req->poweroffset));
        cal_cfg_req->poweroffset_valid = 0x5a;
        BL_DBG("cal_cfg_req->poweroffset is \n%pM\n", cal_cfg_req->poweroffset);
    } else {
        cal_cfg_req->poweroffset_valid = 0x00;
    }
    if (mac_valid) {
        memcpy(cal_cfg_req->mac, mac, sizeof(cal_cfg_req->mac));
        cal_cfg_req->mac_valid = 0x5a;
        BL_DBG("cal_cfg_req->mac is \n%pM\n", cal_cfg_req->mac);
    } else {
        cal_cfg_req->mac_valid = 0x00;
    }
...
#else
    req->type         = bw2chnl[NL80211_CHAN_NO_HT];
                            ...
                            struct apm_start_cfm *cfm, struct bl_dma_elem *elem)
{
...
#else
    req->chan.band = bl_hw->chan->band;
    req->chan.freq = bl_hw->chan->center_freq;
    req->chan.flags = 0;
    req->chan.tx_power = bl_hw->chan->max_power;
    req->center_freq1 = bl_hw->chan->center_freq;
    req->center_freq2 = bl_hw->chan->center_freq;
    req->ch_width = bw2chnl[bl_hw->channel_type];
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)
    req->bcn_int = settings->beacon_interval;
#else
    req->bcn_int = settings->interval;
#endif
    ...
    if (settings->crypto.control_port_ethertype)
        ...
        req->ctrl_port_ethertype = ETH_P_PAE;
#else
    //if (((settings->dtim_period >> 16) & 0xFF) != 1)
    //    flags |= WPA_WPA2_IN_USE;
    //else
    //    flags |= CONTROL_PORT_NO_ENC;
    ...
    printk("AP_START_REQ flags=0x%x\n", flags);
