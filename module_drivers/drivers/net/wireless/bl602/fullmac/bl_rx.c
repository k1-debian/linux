 ...
 void bl_rx_reorder_dispatch_until_start_win(struct bl_hw *bl_hw, struct rxreorder_list *reorder_list, u16 start_win)
{
    ...
    if(count_to_send > reorder_list->win_size)
		count_to_send = reorder_list->win_size;
	
    for (index = 0; index < count_to_send; index++){
		//spin_lock_irq(&reorder_list->rx_lock);
        skb = (struct sk_buff *)reorder_list->reorder_pkt[(index + reorder_list->start_win_index)%reorder_list->win_size];
        reorder_list->reorder_pkt[(index + reorder_list->start_win_index)%reorder_list->win_size] = NULL;		
	...
	reorder_list->start_win_index =(reorder_list->start_win_index + count_to_send)% reorder_list->win_size;	
	reorder_list->end_win = (reorder_list->start_win + reorder_list->win_size -1) % MAX_SEQ_VALUE;
}
 ...
 void bl_rx_reorder_scan_and_dispatch(struct bl_hw *bl_hw, struct rxreorder_list *reorder_list)
{
    ...
    for (index = 0; index < reorder_list->win_size; index++){		
		//spin_lock_irq(&reorder_list->rx_lock);
        skb = (struct sk_buff *)reorder_list->reorder_pkt[(index + reorder_list->start_win_index)%reorder_list->win_size];
        reorder_list->reorder_pkt[(index + reorder_list->start_win_index)%reorder_list->win_size] = NULL;			
		//spin_unlock_irq(&reorder_list->rx_lock);
	...
	reorder_list->start_win_index =(reorder_list->start_win_index + index)% reorder_list->win_size;	
	reorder_list->end_win = (reorder_list->start_win + reorder_list->win_size -1) % MAX_SEQ_VALUE;
}
 ...
 void bl_rx_flush_all(struct bl_hw *bl_hw, struct rxreorder_list *reorder_list)
 {
	...
	for(i=0; i< reorder_list->win_size; i++){
		//spin_lock_irq(&rxreorder_list->rx_lock);
         skb = reorder_list->reorder_pkt[(reorder_list->start_win_index+i)%reorder_list->win_size];
		 reorder_list->reorder_pkt[(reorder_list->start_win_index+i)%reorder_list->win_size] = NULL;		 
			...
			for(i = 0; i < (NX_REMOTE_STA_MAX + NX_VIRT_DEV_MAX); i++){
				for(j = 0; j < NX_NB_TID_PER_STA; j++) {
					if(bl_hw->rx_reorder[i][j].flush) {
						...
						for(k = reorder_list->win_size -1; k > 0 ; k--)
							if(reorder_list->reorder_pkt[(reorder_list->start_win_index + k)%reorder_list->win_size]){
 ...
 void bl_rx_reorder_pkt(struct bl_hw *bl_hw, struct rxreorder_list *reorder_list, u16 sn, u8 is_bar,struct sk_buff *skb)
{
    ...
    if(reorder_list->check_start_win){
        if(sn == reorder_list->start_win) {
            ...
            reorder_list->cnt ++;
            if(reorder_list->cnt < reorder_list->win_size/2){
            ...
            reorder_list->check_start_win = false;
            if (sn != reorder_list->start_win) {
                end_win = (reorder_list->start_win + reorder_list->win_size - 1) & (MAX_SEQ_VALUE -1);
   ...
   win_size = reorder_list->win_size;
   ...
   //1. sn  less than start_win, drop it
	...
	if(!is_bar) {
        ...
        index = (reorder_list->start_win_index + offset) % reorder_list->win_size;
