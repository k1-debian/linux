...
hw_reset:
	...
	gpiod_set_value_cansleep(lp->reset_gpio, 1);
	if (in_atomic()) {
		mdelay(lp->reset_ms);
	} else {
		msleep(lp->reset_ms);
	}
	gpiod_set_value_cansleep(lp->reset_gpio, 0);
...
static int ingenic_mac_set_mac_address(struct net_device *dev, void *p)
{
	...
	memcpy((u8 *)dev->dev_addr, addr->sa_data, dev->addr_len);
	setup_mac_addr(gmacdev, (u8 *)dev->dev_addr);
	return 0;
}
...
static int ingenic_mac_open(struct net_device *dev)
{
	 ...
	 */
	if (!is_valid_ether_addr((u8 *)dev->dev_addr)) {
	...
	if (lp->no_phy_connect)
		/* Mask interrupts triggered by IBS events in the case of no phy */
		synopGMACWriteReg((u32 *)gmacdev->MacBase, GmacInterruptMask,
		                  synopGMACReadReg((u32 *)gmacdev->MacBase, GmacInterruptMask) | GmacRgmiiIntSts);
...
static int ingenic_mac_probe(struct platform_device *pdev)
{
	...
	struct clk *parent = clk_get_parent(lp->clk_cgu);
	unsigned long prate = clk_get_rate(parent);
	unsigned long rate = phy_clk_freq;

	if ((prate % rate) || ((prate / rate) > 256)) {
		parent = clk_get(NULL, "mpll");
		prate = clk_get_rate(parent);
		clk_set_parent(clk_get_parent(lp->clk_cgu), parent);
		if ((prate % rate) || ((prate / rate) > 256)) {
			printk("### ERROR, %s %s %d\n", __FILE__, __func__, __LINE__);
			printk("### ERROR, mpll rate is:%ld can not set rate:%ld\n", prate, rate);
			printk("### ERROR, please check out mpll, then set an appropriate value\n");
		}
	}

	if (clk_set_rate(lp->clk_cgu, rate) || (clk_prepare_enable(lp->clk_cgu))) {
	...
	if (bootargs_ethaddr[pdev->id]) {
		for (i = 0; i < 6; i++) {
			((u8 *)ndev->dev_addr)[i] = ethaddr_hex[pdev->id][i];
			ndev->dev_addr_shadow[i] = ethaddr_hex[pdev->id][i];
		}
	} else {
		random_ether_addr((u8 *)ndev->dev_addr);
		for (i = 0; i < 6; i++) {
			ndev->dev_addr_shadow[i] = ndev->dev_addr[i];
		}
	}
	setup_mac_addr(gmacdev, (u8 *)ndev->dev_addr);
	...
	ndev->min_mtu = MIN_ETHERNET_PAYLOAD;
	ndev->max_mtu = JUMBO_FRAME_PAYLOAD;
	...
	netif_napi_add(ndev, &lp->napi, ingenic_mac_clean);
...
#ifdef CONFIG_INGENIC_MAC_DUMP
	ret = sysfs_create_group(&(pdev->dev.kobj), &mac_debug_attr_group);
	if (ret < 0) {
		dev_err(&pdev->dev, "create group failed! ret = %d\n", ret);
	}
#endif
...
struct ingenic_gmac_priv x2600_priv = {
	.support_multi_if = 0,
	.get_clk_name = get_clk_name_v3,
};
static const struct of_device_id ingenic_mac_dt_match[] = {
	...
	{ .compatible = "ingenic,x2600-mac", .data = &x2600_priv },
	{},
};
