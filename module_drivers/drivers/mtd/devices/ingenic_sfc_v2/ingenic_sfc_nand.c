...
int ingenic_sfcnand_suspend(struct mtd_info *mtd)
{
	return 0;
}

void ingenic_sfcnand_resume(struct mtd_info *mtd)
{
#if defined(CONFIG_SOC_M300) || defined(CONFIG_SOC_X2000)
	struct sfc_flash *flash = to_ingenic_spi_nand(mtd);
	struct ingenic_sfcnand_flashinfo *nand_info = flash->flash_info;
	int ret = 0;
	int fastboot_flag = 0;
	fastboot_flag = pm_get_sleep_level();
	if (fastboot_flag == FASTBOOT) { /*fastboot sleep level*/
...
#define TSHSL_W     100
	...
	flash->mtd._suspend = ingenic_sfcnand_suspend;
	flash->mtd._resume = ingenic_sfcnand_resume;
	...
	if (!flash->pdata_params->use_ofpart_info) {
		...
		if (-EINVAL == ret) {
			ret = mtd_device_parse_register(&flash->mtd, NULL, NULL, NULL, 0);    /* 将整个nand注册成一个完整的mtd分区 */
		} else
		...
		flash->dev->of_node = of_find_node_by_name(NULL, "nandflash");
		if (flash->dev->of_node < 0) {
			dev_err(flash->dev, "Cannot get 'nandflash' node from dtb!\n");
			flash->dev->of_node = NULL;
		}
		mtd_set_of_node(&flash->mtd, flash->dev->of_node);
