...
static int __init ingenic_sfc_probe(struct platform_device *pdev)
{
	...
	switch (flash_type) {
#if (defined(CONFIG_MTD_INGENIC_SFC_NANDFLASH) || defined(CONFIG_MTD_INGENIC_SFC_BOTH_NAND_NOR))
	case NAND:
		ret = ingenic_sfc_nand_probe(flash);
		break;
#endif
#if (defined(CONFIG_MTD_INGENIC_SFC_NORFLASH) || defined(CONFIG_MTD_INGENIC_SFC_BOTH_NAND_NOR))
	case NOR:
...
static int ingenic_sfc_resume(struct platform_device *pdev)
{
	...
	/* 1.To resume from fast boot level sleep, SFC controller initialization is required*/
	if (fastboot_flag == FASTBOOT) {
		sfc_fastboot_resume(pdev);
	} else {
		/* 1.Clk ON */
...
struct sfc_data x2600_sfc_priv = {
	.flash_type_auto_detect = flash_type_auto_detect_from_ddr,
};

struct sfc_data ad100_sfc_priv = {
	.flash_type_auto_detect = flash_type_auto_detect_from_ddr,
};
