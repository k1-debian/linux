...
static int ingenic_sslv_dma_txrx(struct ingenic_sslv *hw, struct spi_ioc_transfer *t)
{
	int ret;
...
__attribute__((__unused__)) static void wait_read(struct ingenic_sslv *hw)
{
	u32 thr;
	/* u32 read_num = hw->len; */
	while (hw->rentries > 0) {
...
__attribute__((__unused__)) static void wait_write(struct ingenic_sslv *hw)
{
	u32 thr;
	while (hw->wentries > 0) {
...
static int ingenic_sslv_pio_txrx(struct ingenic_sslv *hw, struct spi_ioc_transfer *t)
{
	struct ingenic_intr_cnt *g_ingenic_intr = hw->g_ingenic_intr;
	u32 entries = 0, unit_size = 0;
	unsigned long flags;
	int retlen = 0;
	hw->len = t->len;
	hw->wentries = 0;
...
static int ingenic_sslv_setupxfer(struct ingenic_sslv *hw, struct spi_ioc_transfer *t)
{
...
static long ingenic_sslv_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case INGENIC_SPI_DMA_OPS:
			...
			ret = 0;
			break;
		case INGENIC_SPI_MODE0:
			set_polcph_mode(hw, 0);
			ret = 1;
			break;
		case INGENIC_SPI_MODE1:
			set_polcph_mode(hw, 1);
			ret = 1;
			break;
		case INGENIC_SPI_MODE2:
			set_polcph_mode(hw, 2);
			ret = 1;
			break;
		case INGENIC_SPI_MODE3:
			set_polcph_mode(hw, 3);
			ret = 1;
			break;
		default:
	...
	if (t.tx_buf) {
		if ((cnt = copy_from_user(hw->txbuffer, ((void __user *)(uintptr_t)t.tx_buf),
		                          t.len))) {
			printk("cnt:	%d\n", cnt);
			printk("[%s][%d] copy from user error\n",
			       __func__, __LINE__);
	...
	if (t.rx_buf) {
		if (copy_to_user((void __user *)(uintptr_t)t.rx_buf, hw->rxbuffer,
		                 t.len)) {
...
__attribute__((__unused__)) static bool spi_dma_chan_filter(struct dma_chan *chan, void *param)
{
...
static struct platform_driver ingenic_sslvdrv = {
	...
	.driver     = {
		.owner  = THIS_MODULE,
		.name   = "ingenic.slv",
		.of_match_table = ingenic_slv_match,
	},
};
