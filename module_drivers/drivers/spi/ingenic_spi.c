...
static int ingenic_spi_clk_set_rate(struct spi_device *spi, unsigned long rate)
{
	...
	unsigned long cur_rate, real_rate = 0;
	int cgv, ret;
	...
	ingspi->clk_parent = clk_get_parent(ingspi->clk_cgu);
	parent_clk = clk_get_rate(ingspi->clk_parent);
	ret = clk_set_rate(ingspi->clk_cgu, 2 * rate);
	if (ret) {
		printk("---------> clk_set_rate operation failed!!!\n");
		return ret;
	}
	real_rate = clk_get_rate(ingspi->clk_cgu) / 2;/* SSI clock 2 division */
	if (real_rate <= rate) {
		cgv = 0;
	} else {
		if ((real_rate % rate) >= (rate / 2)) {
			cgv = real_rate / rate;
		} else {
			cgv = real_rate / rate - 1;
		}
		if (cgv > 255) {
			cgv = 255;
			printk("-------> Setting frequency too lowi!!!\n");
		}
	}
...
static const struct of_device_id ingenic_spi_match[] = {
	...
	{ .compatible = "ingenic,x2600-spi", .data = (void *) &default_priv },
	{ .compatible = "ingenic,ad100-spi", .data = (void *) &default_priv },
	{}
};
...
static ssize_t ingenic_spi_show_mode(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk("cpu:0\ndma:1\n");
	return 0;
}

static ssize_t ingenic_spi_store_mode(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int mode = 0;
	const char *str = buf;
	struct ingenic_spi *ingspi;

	ingspi = dev_get_drvdata(dev);
	mode = simple_strtoul(str, NULL, 10);
	if (0 == mode) {
		if (0 == ingspi->use_dma) {
			printk("It is already CPU mode!\n");
			return count;
		} else {
			mutex_lock(&ingspi->bitbang.lock);
			ingspi->use_dma = 0;
...
static struct device_attribute spi_master_attribute = __ATTR(mode, S_IRUGO | S_IWUSR, ingenic_spi_show_mode, ingenic_spi_store_mode);
...
static int ingenic_spi_probe(struct platform_device *pdev)
{
	...
	device_create_file(&pdev->dev, &spi_master_attribute);
