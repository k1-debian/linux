...
static inline void set_spi_frame_interval(struct ingenic_spi *spi, unsigned int interval)
{
	u32 tmp;
	tmp = spi_readl(spi, SSI_ITR);
	tmp &= ~(0x7fff << ITR_IVLTM_BIT);
	tmp |= (interval << ITR_IVLTM_BIT);
	spi_writel(spi, SSI_ITR, tmp);
}
