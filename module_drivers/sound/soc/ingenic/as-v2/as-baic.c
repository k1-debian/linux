...
static int ingenic_baic_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	switch (fmt & SND_SOC_DAIFMT_MASTER_MASK) {
		case SND_SOC_DAIFMT_CBS_CFS:
			break;
		case SND_SOC_DAIFMT_CBM_CFM:    /* codec clk provider & frame provider */
...
#ifdef CONFIG_ICODEC_ALWAYS_ON
static int ingenic_baic_probe(struct snd_soc_component *component)
{
	struct snd_soc_dai *dai = NULL;
	struct ingenic_baic *baic = NULL;
	struct ingenic_baic_dai *idai = NULL;

	list_for_each_entry(dai, &component->dai_list, list) {
		if (!strncmp(dai->name, "BAIC0", 5)) {
			baic = dev_get_drvdata(dai->dev);
			idai = &baic->dai[dai->driver->id];
...
void ingenic_baic_remove(struct snd_soc_component *component)
{
	struct snd_soc_dai *dai = NULL;
	struct ingenic_baic *baic = NULL;
	struct ingenic_baic_dai *idai = NULL;
...
static const struct snd_soc_component_driver ingenic_baic_component = {
	.name       = "aic-baic",
#ifdef CONFIG_ICODEC_ALWAYS_ON
	.probe      = ingenic_baic_probe,
	.remove     = ingenic_baic_remove,
#endif
};
...
static int ingenic_baic_platform_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < num_dais; i++) {
		...
		baic->dai_driver[i].symmetric_rate =  baic->dai[i].clk_split ? 0 : 1;
		baic->dai_driver[i].symmetric_channels =  baic->dai[i].clk_split ? 0 : 1;
		baic->dai_driver[i].symmetric_sample_bits =  baic->dai[i].clk_split ? 0 : 1;
