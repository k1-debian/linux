...
static int snd_x2000_probe(struct platform_device *pdev)
{
	...
	icodec_spk_power->spk_gpio = of_get_named_gpio_flags(pdev->dev.of_node, "ingenic,spken-gpio", 0, &flags);
	if (gpio_is_valid(icodec_spk_power->spk_gpio)) {
		unsigned long init_flags;
		icodec_spk_power->spk_en_level = (flags == OF_GPIO_ACTIVE_LOW ? 0 : 1);
