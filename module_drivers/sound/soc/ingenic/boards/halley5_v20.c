...
unsigned int codec_format[MAX_BAIC_NUM] = {
	...
	SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_NB_NF | SND_SOC_DAIFMT_CBS_CFS,
	/*baic4 pcm format*/
	SND_SOC_DAIFMT_DSP_A | SND_SOC_DAIFMT_IB_NF | SND_SOC_DAIFMT_CBM_CFM | BAIC_DAIFMT_SEL_DSP,
};
...
static int spken_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	if (gpio_is_valid(icodec_spk_power->spk_gpio)) {
		if (icodec_spk_power->spk_en_level) {
			ucontrol->value.integer.value[0] = gpio_get_value(icodec_spk_power->spk_gpio);
		} else {
			ucontrol->value.integer.value[0] = !gpio_get_value(icodec_spk_power->spk_gpio);
		}
	}
...
static int spken_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
