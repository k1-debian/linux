	...
	for (i = 0; ingenic_aic->priv[i].clk_name != NULL; i++) {
		if (ingenic_aic->priv[i].clk_mode == AIC_CLOCK_GATE) {
			if (!ingenic_aic->priv[i].is_bus_clk) {
				...
				devm_clk_put(ingenic_aic->dev, clk);
			}
		}
	}
...
static const struct ingenic_aic_priv x2600_priv_data[] = {
	{ .clk_mode = AIC_CLOCK_GATE, .clk_name = "gate_aic",   .is_bus_clk = true },
	{ .clk_mode = AIC_CLOCK_GATE, .clk_name = "gate_i2s",   .is_bus_clk = false },
	{ .clk_mode = AIC_CLOCK_CE,   .clk_name = "ce_i2st" },
	{ .clk_mode = AIC_CLOCK_DIV,  .clk_name = "div_i2s_mn", .stream_mode = AIC_STREAM_COMMON },
	{ .clk_mode = AIC_CLOCK_MUX,  .clk_name = "mux_i2scs",  .mux_select = "epll" },
	{},
};
...
static const struct of_device_id aic_dt_match[] = {
	...
	{ .compatible = "ingenic,x2600-aic", .data = (void *)x2600_priv_data },
	{},
};
