                               ...
                               struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		__i2s_disable_tfifo_loop(aic);
		__i2s_disable_tfifo_loop_dma(aic);
	}
                                        ...
                                        struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		__i2s_enable_tfifo_loop(aic);
		__i2s_enable_tfifo_loop_dma(aic);
	}
...
static int ingenic_i2s_platfrom_probe(struct platform_device *pdev)
{
	...
	ingenic_i2s->dai_driver.name = "i2s-tloop";
