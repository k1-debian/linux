...
#endif
	} else if (2 == icodec->channel) {
		icodec_enable_stereo_record(codec);
	} else {
		dev_err(codec->dev, "%s channel not support channel\n", __func__);
	}
}
                               ...
                               struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(kcontrol->id.name, "Capture Volume")) {
		icodec->alcr_gain = ucontrol->value.integer.value[0];
		if (snd_soc_component_active(component)) {
			icodec_reg_set(icodec, TS_CODEC_CANACR_26, 0, 4, icodec->alcr_gain);
			icodec_reg_set(icodec, TS_CODEC_CANACR2_27, 0, 4, icodec->alcr_gain);
		}
	}
...
static const struct snd_kcontrol_new icodec_controls[] = {
	...
	SOC_SINGLE_EXT_TLV("Capture Volume", TS_CODEC_CANACR2_27, 0, 0x1f, 0, icodec_controls_get, icodec_controls_put, alcr_gain_tlv),
	...
	SOC_SINGLE_EXT_TLV("Speaker Playback Volume", TS_CODEC_CHR_2b, 0, 0x1f, 0, icodec_controls_get, icodec_controls_put, hpoutl_again_tlv),
...
static int icodec_platform_probe(struct platform_device *pdev)
{
	...
	icodec->spk_gpio = of_get_named_gpio(pdev->dev.of_node, "ingenic,spken-gpio", 0);
	icodec->sgm8903_gpio = of_get_named_gpio(pdev->dev.of_node, "ingenic,sgm8903en-gpio", 0);
	if (gpio_is_valid(icodec->spk_gpio) && gpio_is_valid(icodec->sgm8903_gpio)) {
		unsigned long init_flags;
		icodec->spk_en_level = 1;
