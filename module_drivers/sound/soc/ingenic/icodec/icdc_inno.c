...
static void enter_standby_state(void)
{
	int reg = 0;
...
static void exit_standby_state(void)
{
	int reg = 0;
	reg  = *(volatile unsigned int *)(INNO_CODEC_BASE + 0x26 * 4) ;
	reg &= ~(0x3 << 0);
...
static void inno_poweron(struct snd_soc_component *component)
{
	...
	*(volatile unsigned int *)(INNO_CODEC_BASE + 0x22 * 4) = reg;
	...
	for (i = 0; i < 8; i++) {
		*(volatile unsigned int *)(INNO_CODEC_BASE + 0x21 * 4) |= 1 << i;
		mdelay(20);
	}
	*(volatile unsigned int *)(INNO_CODEC_BASE + 0x21 * 4) = 0xa0;
	mdelay(10);
...
static void inno_dac_enable(struct snd_soc_component *component)
{
	...
	*(volatile unsigned int *)(INNO_CODEC_BASE + 0x26 * 4) = 0;
	mdelay(30);
#endif
	reg  = *(volatile unsigned int *)(INNO_CODEC_BASE + 0x20 * 4) ;
	reg &= ~(0xf << 0);
	reg |= (7 << 0);
	*(volatile unsigned int *)(INNO_CODEC_BASE + 0x20 * 4) = reg;
	mdelay(10);
...
static void inno_poweroff(struct snd_soc_component *component)
{
	...
	*(volatile unsigned int *)(INNO_CODEC_BASE + 0x21 * 4) = 0;
	mdelay(10);
...
{
#ifndef CONFIG_ICODEC_ALWAYS_ON
                               ...
                               struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(kcontrol->id.name, "Digital Volume") || \
	    !strcmp(kcontrol->id.name, "Speaker Playback Volume")) {
		ucontrol->value.integer.value[0] = icodec->digtal_gain;
	}

	if (!strcmp(kcontrol->id.name, "AMIC Capture Volume")) {
		ucontrol->value.integer.value[0] = icodec->alcl_gain;
	}
                               ...
                               struct snd_ctl_elem_value *ucontrol)
{
	...
	int i = 0;
	int step = 0;
	unsigned int digtal_gain = 0;

	if (!strcmp(kcontrol->id.name, "ICODEC MICL GAIN")
	    || !strcmp(kcontrol->id.name, "AMIC Capture Volume")) {
	...
	if (!strcmp(kcontrol->id.name, "Digital Volume") || \
	    !strcmp(kcontrol->id.name, "Speaker Playback Volume")) {
		icodec->digtal_gain = ucontrol->value.integer.value[0];
		if (icodec->digtal_gain) {
			/* Digital Gain*/
			reg  = *(volatile unsigned int *)(INNO_CODEC_BASE + 0x6 * 4) ;
			digtal_gain = reg & (0x7 << 0);
			reg &= ~(0x7 << 0);
			step = digtal_gain > (8 - icodec->digtal_gain) ? (-1) : 1;
			for (i = digtal_gain; digtal_gain != (8 - icodec->digtal_gain);) {
				digtal_gain += step;
				*(volatile unsigned int *)(INNO_CODEC_BASE + 0x6 * 4) = (reg | digtal_gain);
				mdelay(i * 15);
			}
		...
		if (!icodec->digtal_gain) {
			icodec_mute = 1;
			reg = *(volatile unsigned int *)(INNO_CODEC_BASE + 0x7 * 4);
			reg &= ~(0x1 << 3);
			reg |= (0x1 << 3);
			*(volatile unsigned int *)(INNO_CODEC_BASE + 0x7 * 4) = reg;
			mdelay(20);

		} else if (icodec_mute == 1 && icodec->digtal_gain) {
			reg = *(volatile unsigned int *)(INNO_CODEC_BASE + 0x7 * 4);
			reg &= ~(0x1 << 3);
			*(volatile unsigned int *)(INNO_CODEC_BASE + 0x7 * 4) = reg;
			mdelay(10);
...
/*
 * Digital Gain Volume control:
 * from -30dB to 12dB in 6dB steps
 * */
static const DECLARE_TLV_DB_SCALE(digtal_gain_tlv, -3000, 600, 0);
...
static const struct snd_kcontrol_new icodec_controls[] = {
	...
	SOC_SINGLE_EXT_TLV("AMIC Capture Volume", 0, 0, 0x1f, 0, icodec_controls_get, icodec_controls_put, alcl_gain_tlv),
	SOC_SINGLE_EXT_TLV("Speaker Playback Volume", 0, 0, 0x6, 0, icodec_controls_get, icodec_controls_put, digtal_gain_tlv),
	SOC_SINGLE_EXT_TLV("Digital Volume", 0, 0, 0x6, 0, icodec_controls_get, icodec_controls_put, digtal_gain_tlv),
#else
	...
	SOC_SINGLE_TLV("Speaker Playback Volume", 0x6 * 4, 0, 0x6, 0, digtal_gain_tlv),
	SOC_SINGLE_TLV("Digital Volume", 0x6 * 4, 0, 0x6, 0, digtal_gain_tlv),
