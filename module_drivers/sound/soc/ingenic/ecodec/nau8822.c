...
static const struct snd_kcontrol_new nau8822_snd_controls[] = {
	...
	SOC_SINGLE("Right Speaker Submixer Bypass Control",
	           NAU8822_REG_RIGHT_SPEAKER_CONTROL, 4, 1, 0),

	SOC_SINGLE("Left DAC Output To RMIX Right Output Mixer",
	           NAU8822_REG_OUTPUT_CONTROL, 6, 1, 0),

	SOC_SINGLE("Right DAC Output To RMIX Right Output Mixer",
...
{
	struct snd_soc_component *codec = snd_soc_dapm_to_component(source->dapm);
	struct nau8822_priv *nau8822 = snd_soc_component_get_drvdata(codec);
	unsigned int value;
...
static int nau8822_set_bias_level(struct snd_soc_component *codec,
...
static int nau8822_suspend(struct snd_soc_component *codec)
{
	struct nau8822_priv *nau8822 = snd_soc_component_get_drvdata(codec);
...
static int nau8822_resume(struct snd_soc_component *codec)
{
	struct nau8822_priv *nau8822 = snd_soc_component_get_drvdata(codec);
...
static int nau8822_probe(struct snd_soc_component *codec)
{
	struct nau8822_priv *nau8822 = snd_soc_component_get_drvdata(codec);
	int i;
...
static void nau8822_remove(struct snd_soc_component *codec)
{
	nau8822_set_bias_level(codec, SND_SOC_BIAS_OFF);

}

static struct snd_soc_component_driver soc_codec_dev_nau8822 = {
	.probe =    nau8822_probe,
...
static int nau8822_i2c_probe(struct i2c_client *i2c)
{
	...
	/* 0x2B RSUBBYP bit[] */
	regmap_update_bits(nau8822->regmap, NAU8822_REG_RIGHT_SPEAKER_CONTROL, (1 << 4), (1 << 4));
	regmap_update_bits(nau8822->regmap, NAU8822_REG_OUTPUT_CONTROL, (1 << 6), (1 << 6));
	regmap_update_bits(nau8822->regmap, NAU8822_REG_RIGHT_MIXER_CONTROL, (1 << 0), 0x0);

	ret = snd_soc_register_component(dev, &soc_codec_dev_nau8822, &nau8822_dai, 1);
	if (ret != 0) {
...
static void nau8822_i2c_remove(struct i2c_client *client)
{
