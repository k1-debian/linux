...
int efi_partition(struct parsed_partitions *state)
{
	...
	for (i = 0; i < le32_to_cpu(gpt->num_partition_entries) && i < state->limit-1; i++) {
		...
		unsigned label_max;
		u64 disk_size = get_capacity(state->disk);
		u64 start = le64_to_cpu(ptes[i].starting_lba);
		u64 end = le64_to_cpu(ptes[i].ending_lba);
		if (end == 0xFFFFFFFFFFFFFFFF || end > disk_size) {
			ptes[i].ending_lba = disk_size - 1ULL;
			pr_warn("Adjusted to use the entire disk size %llu", le64_to_cpu(ptes[i].ending_lba));
		}

		u64 size = end - start + 1ULL;

		pr_debug("%s >>>>> (%lld - %lld),size = %lld,i = %d\n",__FUNCTION__,le64_to_cpu(ptes[i].starting_lba),le64_to_cpu(ptes[i].ending_lba), size, i);
